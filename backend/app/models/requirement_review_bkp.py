...
class RequirementQualityReview(TimestampMixin, Base):
    ...
    interface_readiness_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    scenario_generation_readiness: Mapped[float | None] = mapped_column(Float, nullable=True)
    qa_domain_completeness: Mapped[float | None] = mapped_column(Float, nullable=True)
