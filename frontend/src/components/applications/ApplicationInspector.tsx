...
function InspectorCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mb-4 rounded-lg border border-slate-200 bg-white p-4">
      <h3 className="mb-3 text-xs font-extrabold text-slate-900">{title}</h3>
      {children}
    </section>
  );
}
...
function NotConfigured({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-dashed border-slate-200 bg-slate-50/60 px-3 py-2 text-[11px] font-semibold text-slate-400">
  ...
  return (
    ...
    <>
      <DrawerHeader>
        ...
        <button aria-label="Close" onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-50"><X className="h-4 w-4" /></button>
      </DrawerHeader>
      <DrawerBody>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={cn(
              ...
              tab === t.key ? "bg-[#07142d] text-white" : "text-slate-500 hover:bg-slate-100",
            )}
          >
            {t.label}
          </button>
      ...
      {tab === "overview" && (
        <>
          ...
          <InspectorCard title="Identity">
            ...
            {app.description && <p className="mt-3 text-[11px] font-semibold leading-relaxed text-slate-600">{app.description}</p>}
          </InspectorCard>
          ...
          {canManage && (
            <Button size="sm" onClick={onEdit} className="h-9 w-full bg-[#1b59f8] text-xs font-bold text-white hover:bg-blue-700">
              Edit Application
            </Button>
          )}
        </>
      ...
      {tab === "environments" && (
        <InspectorCard title="Configured Environments">
          {Object.keys(app.environment_urls || {}).length === 0 ? (
            <p className="text-xs font-semibold text-slate-400">No environments configured.</p>
          ) : (
            <div className="space-y-3">
              {Object.entries(app.environment_urls).map(([env, url]) => (
                <div key={env} className="rounded-lg border border-slate-200 p-3">
                  ...
                  <p className="flex items-center gap-1 truncate text-[11px] font-semibold text-slate-600"><Globe className="h-3 w-3 shrink-0" />{url}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <>
          ...
          <InspectorCard title="Usage">
            <div className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
              <span className="text-[11px] font-bold text-slate-600">Targeting test cases</span>
      ...
      {tab === "dependencies" && (
        <InspectorCard title="External Dependencies">
          {dependencies.length === 0 ? (
            <p className="text-xs font-semibold text-slate-400">No external dependencies recorded for this application.</p>
          ) : (
            <div className="space-y-2">
              {dependencies.map((dep) => (
                <div key={dep.id} className="rounded-lg border border-slate-200 p-3">
                  <div className="mb-1 flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-xs font-bold text-slate-800"><Plug className="h-3.5 w-3.5 text-slate-400" />{dep.service_name}</span>
                    <Badge variant={dep.is_active ? "success" : "secondary"}>{dep.is_active ? "Active" : "Inactive"}</Badge>
                  </div>
                  <p className="text-[10px] font-bold uppercase text-slate-400">Mock Strategy: <span className="text-slate-600">{dep.mock_strategy}</span></p>
                  {dep.sandbox_url && <p className="truncate text-[11px] font-semibold text-slate-600">{dep.sandbox_url}</p>}
                  {dep.note && <p className="mt-1 text-[11px] font-semibold text-slate-500">{dep.note}</p>}
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "discovery" && (
        <>
          <InspectorCard title="Discovery Readiness (Proxy)">
            <div className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
              <span className="text-[11px] font-bold text-slate-600">Active + environment configured</span>
            ...
            <p className="mt-2 text-[10px] font-semibold text-slate-400">Environment-readiness proxy — not real discovery-session telemetry.</p>
          </InspectorCard>
      ...
      {tab === "history" && (
        <InspectorCard title="Change History">
          {events.length === 0 ? (
            <p className="text-xs font-semibold text-slate-400">No recorded changes for this application.</p>
          ) : (
            <div className="space-y-3">
              {events.map((event, idx) => (
                <div key={idx} className="rounded-lg border border-slate-100 p-2.5">
                  ...
                  <p className="mb-1 text-[10px] font-extrabold text-slate-700">{resolveUserName(userNames, event.entry.changed_by)}</p>
                  {event.isDependency ? (
                    <p className="text-[10px] font-semibold text-slate-600">External dependency configuration changed.</p>
                  ) : (
                    <ul className="space-y-1">
                      {event.changes.map((c, i) => (
                        <li key={i} className="text-[10px] font-semibold text-slate-600">
                          <span className="font-bold text-slate-800">{fieldLabel(c.field)}:</span> {formatValue(c.oldValue)} → {formatValue(c.newValue)}
                        </li>
                      ))}
                    </ul>
                  )}
                  {event.entry.change_reason && <p className="mt-1 text-[10px] italic text-slate-500">&quot;{event.entry.change_reason}&quot;</p>}
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "activity" && (
        <InspectorCard title="Activity Feed">
          {events.length === 0 ? (
            <p className="text-xs font-semibold text-slate-400">No recorded activity for this application.</p>
          ) : (
            <div className="space-y-3">
              {events.map((event, idx) => (
                <div key={idx} className="relative pl-4">
                  <span className="absolute left-0 top-1.5 h-1.5 w-1.5 rounded-full bg-[#1b59f8]" />
                  <p className="text-[9px] font-bold text-slate-400">{new Date(event.entry.changed_at).toLocaleString()}</p>
                  <p className="mt-0.5 text-[11px] font-semibold text-slate-700">{activitySentence(event, userNames)}</p>
                </div>
              ))}
            </div>
          )}
