    ...
    >
      ...
      <p
        className={cn(
          ...
          missing ? "text-gray-300" : "text-gray-950",
        )}
      >
        {missing ? "—" : value}
      </p>
...
export function Panel({
  ...
  return (
    <section className={cn("rounded-lg border border-gray-200 bg-white p-3", className)}>
      <div className="mb-2 flex items-center justify-between gap-2">
        <h3 className="text-[10px] font-extrabold uppercase tracking-wide text-gray-800">{title}</h3>
        {action}
      </div>
      {children}
    </section>
...
export function DisabledAction({
  ...
  return (
    <button
      ...
      title={reason}
      className={cn(
        "flex w-full cursor-not-allowed items-start gap-2.5 rounded-lg border border-dashed border-gray-200 bg-gray-50/60 px-3 py-2.5 text-left",
        className,
      )}
    >
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-400">
        {Icon ? <Icon className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
      </span>
      <span className="min-w-0">
        <span className="block truncate text-xs font-bold text-gray-500">{label}</span>
        <span className="block text-[10px] font-semibold text-gray-400">{reason}</span>
      </span>
    </button>
  );
}
...
export function DisabledTab({ label, reason }: { label: string; reason: string }) {
  return (
    <button
      ...
      disabled
      title={reason}
      className="flex cursor-not-allowed items-center gap-1 whitespace-nowrap border-b-2 border-transparent px-2.5 py-2 text-[10px] font-bold text-gray-300"
    >
...
export function EmptyRow({ colSpan, message }: { colSpan: number; message: string }) {
  return (
    <tr>
      <td colSpan={colSpan} className="p-6 text-center text-[10px] font-semibold text-gray-400">
        {message}
      </td>
    </tr>
  );
