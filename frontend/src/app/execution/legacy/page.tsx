...
type ManualStepState = {
  ...
  updatedAt?: string;
  /** Server-side ManualStepResult.id. Set once the run is started on the backend. */
  stepId?: number;
};
...
type AiWorkspaceStatus = "draft" | "plan_ready" | "approved" | "in_progress" | "needs_review" | "paused" | "completed" | "failed";
...
function modeForRun(run: ExecutionRun): ExecutionTab {
  const metadataSource = typeof run.metadata_?.source_type === "string" ? run.metadata_.source_type : "";
  const source = (run.source_type || metadataSource || "").toLowerCase();
...
function createWorkspaceAdapterRun(projectId: number, tab: ExecutionTab, testCases: TestCase[]): ExecutionRun {
  ...
  const prefix = tab === "manual" ? "MR" : "AUT";
  const sourceType = tab === "manual" ? "manual_adapter" : "automation_adapter";
  return {
...
function ensureWorkspaceCatalogRuns(projectId: number | null, runs: ExecutionRun[], testCases: TestCase[]) {
  ...
  const tabs: ExecutionTab[] = ["manual", "automation"];
...
function frameworkLabelFromScript(script?: AutomationScript | null) {
  const framework = (script?.framework || "").toLowerCase();
  if (framework === "playwright") return "Playwright";
  if (framework === "pytest") return "pytest";
  return script?.framework || "Automation";
}

function repositoryMeta(script?: AutomationScript | null) {
  const metadata = script?.metadata_ && typeof script.metadata_ === "object" ? script.metadata_ : {};
  return {
    repository: typeof metadata.repository === "string" ? metadata.repository : "Repository pending",
    branch: typeof metadata.branch === "string" ? metadata.branch : "codex/automation-drafts",
  };
}

function approvedAutomationScriptRuns(
  projectId: number | null,
  scripts: AutomationScript[],
  testCases: TestCase[],
  savedRuns: ExecutionRun[] = [],
) {
  if (!projectId) return [];
  const testCaseById = new Map(testCases.map((testCase) => [testCase.id, testCase]));
  return scripts
    .filter((script) => script.status === "approved" && script.test_case_id)
    .sort((left, right) => left.script_id.localeCompare(right.script_id, undefined, { numeric: true, sensitivity: "base" }))
    .map((script) => {
      const testCase = script.test_case_id ? testCaseById.get(script.test_case_id) : undefined;
      const meta = repositoryMeta(script);
      const now = script.updated_at || script.created_at || new Date().toISOString();
      const projectedRun = {
        id: -(projectId * 100000 + script.id),
        project_id: projectId,
        execution_id: `AUT-${script.script_id}`,
        source_type: "automation_adapter",
        external_tool_name: frameworkLabelFromScript(script),
        suite_name: testCase?.test_case_id ? `${testCase.test_case_id} Automation Draft` : script.file_path || script.script_id,
        environment: "SIT",
        status: "queued",
        total_tests: 1,
        passed: 0,
        ...
        execution_logs: ["Approved automation script is ready for orchestration from Execution."],
        metadata_: {
          adapter_backed: true,
          ...
          branch: meta.branch,
          target_url: "Use Start Execution to hand this approved script to a runner.",
          note: "Approved automation script projected into Execution before first live backend run.",
        },
        created_at: script.created_at,
        updated_at: now,
      } satisfies ExecutionRun;

      const savedRun = savedRuns.find((row) => {
        const savedAutomationScriptId = typeof row.metadata_?.automation_script_id === "number"
          ? row.metadata_.automation_script_id
          : null;
        return row.id === projectedRun.id
          || row.execution_id === projectedRun.execution_id
          || savedAutomationScriptId === script.id;
      });

      if (!savedRun) return projectedRun;

      const savedMetadata = savedRun.metadata_ && typeof savedRun.metadata_ === "object" ? savedRun.metadata_ : {};
      const projectedMetadata = projectedRun.metadata_ && typeof projectedRun.metadata_ === "object" ? projectedRun.metadata_ : {};

      return {
        ...projectedRun,
        status: savedRun.status || projectedRun.status,
        total_tests: savedRun.total_tests ?? projectedRun.total_tests,
        passed: savedRun.passed ?? projectedRun.passed,
        failed: savedRun.failed ?? projectedRun.failed,
        skipped: savedRun.skipped ?? projectedRun.skipped,
        started_at: savedRun.started_at ?? projectedRun.started_at,
        completed_at: savedRun.completed_at ?? projectedRun.completed_at,
        duration_seconds: savedRun.duration_seconds ?? projectedRun.duration_seconds,
        execution_logs: Array.isArray(savedRun.execution_logs) && savedRun.execution_logs.length > 0
          ? savedRun.execution_logs
          : projectedRun.execution_logs,
        suite_name: savedRun.suite_name || projectedRun.suite_name,
        environment: savedRun.environment || projectedRun.environment,
        external_tool_name: savedRun.external_tool_name || projectedRun.external_tool_name,
        created_at: savedRun.created_at || projectedRun.created_at,
        updated_at: savedRun.updated_at || projectedRun.updated_at,
        metadata_: {
          ...savedMetadata,
          ...projectedMetadata,
        },
...
function normalizeExecutionEnvironment(value?: string | null): "local" | "staging" | "production" | "ci" {
  const normalized = (value || "").toLowerCase();
  if (normalized === "local" || normalized === "dev" || normalized === "development") return "local";
...
function deterministicAutomationOutcome(run: ExecutionRun) {
  const seed = `${run.execution_id}:${String(run.metadata_?.script_id || run.metadata_?.script_path || run.suite_name || "")}`;
  const checksum = seed.split("").reduce((sum, char) => sum + char.charCodeAt(0), 0);
  return checksum % 5 === 0 ? "failed" : "passed";
}

function createScriptAdapterResults(run: ExecutionRun, scripts: AutomationScript[], testCases: TestCase[]): ExecutionResult[] {
  const scriptId = typeof run.metadata_?.automation_script_id === "number" ? run.metadata_.automation_script_id : null;
  if (!scriptId) return [];
  const script = scripts.find((row) => row.id === scriptId);
  if (!script) return [];
  const testCase = script.test_case_id ? testCases.find((row) => row.id === script.test_case_id) : undefined;
  const steps = testCase ? fallbackStepsForTestCase(testCase) : [{ step_number: 1, action: script.file_path || script.script_id, expected_result: "Runner executes approved automation draft." }];
  const meta = repositoryMeta(script);
  const now = run.updated_at || run.created_at || new Date().toISOString();
  return [{
    id: run.id * 1000 - 1,
    execution_run_id: run.id,
    test_case_id: testCase?.id,
    test_name: testCase?.title || script.file_path || script.script_id,
    status: "not_run",
    execution_mode: "automated",
    external_tool_name: frameworkLabelFromScript(script),
    external_test_case_id: testCase?.external_tc_id || testCase?.test_case_id || script.script_id,
    automation_execution_status: "not_run",
    ...
    raw_result_json: {
      ...
      repository: meta.repository,
      branch: meta.branch,
      source: "approved_automation_script",
      step_count: steps.length,
    },
    logs: ["Approved automation script projected into Execution. Live evidence will appear after runner invocation."],
    created_at: now,
    updated_at: now,
  }];
}
...
/** Map both DB vocab (pass/fail/skip) and long vocab (passed/failed/skipped) to a single token. */
function normalizeResultStatus(raw?: string | null): "passed" | "failed" | "blocked" | "skipped" | "error" | "not_run" | "running" | "pending" | "" {
...
function resultStatusCounts(results: ExecutionResult[], manualStepStates?: Record<string, ManualStepState>, testCaseById?: Map<number, TestCase>) {
  ...
  results.forEach((result) => {
    let status = normalizeResultStatus(result.manual_execution_status || result.automation_execution_status || result.status);
    if (result.id < 0 && manualStepStates && testCaseById) {
      ...
      status = normalizeResultStatus(manualResultStatus(summary));
    }
...
function applyManualRunDetailToStates(
  detail: ManualRunDetail,
  prev: Record<string, ManualStepState>,
): Record<string, ManualStepState> {
  const next: Record<string, ManualStepState> = { ...prev };
  for (const result of detail.results) {
    for (const step of result.steps) {
      next[manualStepKey(result.result.id, step.step_number)] = {
        status: step.status,
...
function frameworkForRun(run?: ExecutionRun, results: ExecutionResult[] = []) {
  const metaFramework = typeof run?.metadata_?.framework === "string" ? run.metadata_.framework : "";
  const value = metaFramework || run?.external_tool_name || results.find((result) => result.external_tool_name)?.external_tool_name || "";
  const normalized = value.toLowerCase();
...
function automationRunStateFor(run: ExecutionRun, state?: AutomationRunState): AutomationRunState {
  ...
  // Map server-side status to the frontend's adapter status vocabulary.
  const raw = (run.status || "draft").toLowerCase().replace(/\s+/g, "_");
  const serverToAdapter: Record<string, AutomationRunState["status"]> = {
    pending: "queued",
    queued: "queued",
    running: "in_progress",
    in_progress: "in_progress",
    completed: (run.failed || 0) > 0 ? "failed" : "passed",
    failed: "failed",
    cancelled: "aborted",
    draft: "draft",
  };
  const mappedStatus = serverToAdapter[raw] || "draft";
  const failedCount = run.failed || 0;
  const passedCount = run.passed || 0;
  const skippedCount = run.skipped || 0;
  const messageByStatus: Record<string, string> = {
    queued: `${run.execution_id} queued on the local subprocess runner.`,
    in_progress: `${run.execution_id} is running on the local subprocess runner — polling for completion.`,
    failed: failedCount > 0
      ? `${run.execution_id} finished with ${failedCount} failed test(s). See the result row for the error and the run log.`
      : `${run.execution_id} did not complete successfully. Check the result row and the run log for details.`,
    passed: `${run.execution_id} completed: ${passedCount} passed, ${failedCount} failed, ${skippedCount} skipped.`,
    aborted: `${run.execution_id} was cancelled before completing.`,
    draft: "Click Start to dispatch this script to the local subprocess runner.",
  };
  return {
    status: mappedStatus,
    message: messageByStatus[mappedStatus] || messageByStatus.draft,
    currentStep: mappedStatus === "in_progress" ? "Running" : mappedStatus === "queued" ? "Queued" : "",
...
function automationFailureInsight(results: ExecutionResult[]) {
  const failed = results.find((result) => ["failed", "error"].includes(normalizeResultStatus(result.automation_execution_status || result.status)));
...
const automationExecutionService = {
  async startAutomationRun(run: ExecutionRun) {
    return {
      ...
      message: `${run.execution_id} execution started in the local orchestration workspace. Live runner integration can replace this handoff later.`,
      currentStep: "Launching automation runner",
    };
  },
...
function aiStatusFromExecutionRun(run: ExecutionRun): AiWorkspaceStatus {
  const status = (run.status || "").toLowerCase();
  if (["failed", "error", "aborted", "cancelled"].includes(status)) return "failed";
  if (["queued", "pending", "running", "in_progress"].includes(status)) return "in_progress";
  if (["completed", "passed"].includes(status)) {
    if ((run.total_tests || 0) === 0 || (run.failed || 0) > 0) return "needs_review";
    return "completed";
  }
  return "draft";
}
...
function aiWorkspaceStateFor(run: ExecutionRun, state?: AiWorkspaceState): AiWorkspaceState {
  if (state) return state;
  const persistedAiRun = run.id > 0 && isAiAssisted(run);
  const derivedStatus = persistedAiRun ? aiStatusFromExecutionRun(run) : "draft";
  const hasTerminalRun = ["completed", "failed", "needs_review"].includes(derivedStatus);
  const passRate = safePercent(run.passed || 0, run.total_tests || 0);
  return {
    mode: "Assistive",
    status: derivedStatus,
    planGenerated: persistedAiRun,
    approved: persistedAiRun,
    confidence: persistedAiRun ? passRate : 0,
    executionSteps: [],
    browserActions: [],
    validations: [],
    testData: [],
    selectorSuggestions: [],
    risks: hasTerminalRun ? [] : ["AI execution has not been planned yet. Human review is required before any action."],
    observations: persistedAiRun
      ? [
          `${run.execution_id} persisted with ${run.passed || 0} passed, ${run.failed || 0} failed, and ${run.skipped || 0} skipped.`,
          ...(run.execution_logs || []).slice(0, 4).map((log) => String(log)),
        ]
      : (run.execution_logs || []).slice(0, 4).map((log) => String(log)),
    lastUpdated: run.updated_at,
  };
...
function ManualWorkspace({
  ...
  onAttachEvidence: (result: ExecutionResult, stepNumber: number, file: File) => void;
  ...
  // "Ask AI" — per-step suggestion state, scoped to the current step so changing
  // step or test case clears the panel.
  ...
  async function handleAskAi(useScreenshot: boolean) {
    if (!currentState.stepId || !currentStep || !selectedResult) return;
    setAiBusy(true);
    setAiError(null);
    try {
      let screenshot: File | undefined;
      if (useScreenshot && currentState.evidence.length > 0) {
        const latest = currentState.evidence[currentState.evidence.length - 1];
        try {
          const res = await fetch(latest.download_url, { credentials: "include" });
          if (res.ok) {
            const blob = await res.blob();
            screenshot = new File([blob], latest.filename, { type: blob.type || "application/octet-stream" });
          }
        } catch {
      ...
      const mappedStatus: ManualStepStatus =
        ...
        r.data.suggested_status === "fail" ? "failed" : "blocked";
      setAiSuggestion({
        ...
        mode: r.data.inputs_used?.mode || "text_only",
      });
    } catch (err) {
      setAiError(messageFromError(err, "Could not get an AI suggestion."));
    } finally {
      setAiBusy(false);
    }
  }
  ...
  function applyAiSuggestion() {
    if (!aiSuggestion || !currentStep || !selectedResult) return;
    onUpdateStep(
      selectedResult,
      currentStep.step_number,
      { status: aiSuggestion.status },
      `Step ${currentStep.step_number} marked ${normalizeStatus(aiSuggestion.status)} via AI assist (confidence ${aiSuggestion.confidence}%)`,
    );
    setAiSuggestion(null);
  }
              ...
              <div className="rounded-lg border border-gray-200">
                ...
                <div className="space-y-4 p-4">
                  ...
                  <div>
                    ...
                    {aiError && (
                      <div className="mt-3 flex items-start gap-2 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] font-semibold text-rose-700">
                        <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                        <span className="flex-1">{aiError}</span>
                        <button onClick={() => setAiError(null)} className="text-rose-400 hover:text-rose-700"><XCircle className="h-3.5 w-3.5" /></button>
                      </div>
                    )}

                    {aiSuggestion && aiSuggestion.stepId === currentState.stepId && (
                      <div className={cn(
                        "mt-3 rounded-lg border p-3",
                        aiSuggestion.status === "passed" && "border-emerald-200 bg-emerald-50/60",
                        aiSuggestion.status === "failed" && "border-rose-200 bg-rose-50/60",
                        aiSuggestion.status === "blocked" && "border-amber-200 bg-amber-50/60",
                      )}>
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <Sparkles className={cn(
                              "h-4 w-4",
                              aiSuggestion.status === "passed" && "text-emerald-600",
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          >
                            Apply as {normalizeStatus(aiSuggestion.status)}
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                    ...
                    <div className="flex flex-wrap gap-2">
                      <label className="inline-flex h-8 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-xs font-medium text-gray-700 transition hover:bg-gray-50">
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          disabled={!currentState.stepId}
                          onChange={(event) => {
                            const file = event.target.files?.[0];
                            if (file) onAttachEvidence(selectedResult, currentStep.step_number, file);
                            event.target.value = "";
                          }}
                        />
                      </label>
    ...
    <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div className="min-w-0 space-y-4">
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-[11px] font-semibold leading-relaxed text-amber-800">
          Script generation, framework recommendation, and review now belong in the Automation module. This Execution workspace stays focused on runner orchestration, live evidence, reruns, and failure handling.
        </div>
      </div>
              ...
              <div className="mt-4 rounded-lg bg-gray-950 p-3 font-mono text-[10px] leading-relaxed text-gray-200">
                ...
                {streamRows.length ? streamRows.slice(0, 3).map((result) => (
                  <p key={result.id} className={cn(
                    "truncate",
                    ["failed", "error"].includes(normalizeResultStatus(result.status)) ? "text-rose-300" : "text-emerald-300"
                  )}>
...
function AiWorkspace({
  ...
  onGeneratePlan: () => void;
  onApprovePlan: () => void;
  onRunAiExecution: () => void | Promise<void>;
  onPause: () => void;
...
function WorkspaceShell({
  ...
  onAttachManualEvidence: (result: ExecutionResult, stepNumber: number, file: File) => void;
  onRemoveManualEvidence: (evidenceId: string) => void;
  onCreateManualDefect: (result: ExecutionResult, stepNumber: number) => void;
  onStartManualRun: (adapterRun: ExecutionRun) => void;
  startingManualRun: boolean;
  creatingDefectKey: string | null;
  ...
  onRunAiExecution: (run: ExecutionRun) => void | Promise<void>;
  onRunAiTestCase: (testCase?: TestCase) => void | Promise<void>;
    ...
    return (
      <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <Button size="sm" className="h-8 text-xs font-bold" disabled={!selectedTestCase} onClick={() => onRunAiTestCase(selectedTestCase)}>
            <Play className="h-3.5 w-3.5" /> Run AI Test
          </Button>
        </div>
        {selectedTestCase ? (
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <MetaTile label="Selected Test Case" value={selectedTestCase.test_case_id || selectedTestCase.title} />
            <MetaTile label="Execution Mode" value={selectedTestCase.execution_mode || selectedTestCase.mode || "AI"} />
            <MetaTile label="Expected Result" value={selectedTestCase.expected_result || "Not provided"} />
            <MetaTile label="Guardrail" value="Backend run required; no local AI mock run is created." />
          </div>
        ) : (
          <EmptyState title="No AI test case ready" description="Set a test case execution mode to AI or Hybrid and approve it before running AI execution." />
        )}
      </section>
    );
  }
        ...
        {isAdapterRun ? (
          <div className="m-4 flex flex-col gap-3 rounded-lg border border-app-brand-200 bg-app-brand-75/60 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button
              ...
              disabled={startingManualRun || selectedResults.length === 0}
              onClick={() => onStartManualRun(selectedRun)}
            >
              {startingManualRun ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
              Start Manual Run
            </Button>
          </div>
        ) : (
          <ManualWorkspace
            ...
            onRemoveEvidence={onRemoveManualEvidence}
...
function ExecutionContent() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [runsRes, casesRes, scriptsRes] = await Promise.all([
        executionApi.listRuns(selectedProject),
        testCasesApi.list(selectedProject),
        automationApi.list(selectedProject, { status: "approved" }),
      ]);
      ...
      const approvedScriptRuns = approvedAutomationScriptRuns(selectedProject, scriptsRes.data, casesRes.data, localRuns);
      const tabs: ExecutionTab[] = ["manual"];
      // Always regenerate adapter runs from current data — they're CTAs, not
      // persisted state, so we never trust localStorage for them.
      const adapters = tabs
        .filter((tab) => !backendRuns.some((run) => modeForRun(run) === tab))
        .map((tab) => {
          if (tab === "automation") return null;
      ...
      const allRuns = [...backendRuns, ...approvedScriptRuns, ...adapters];
      setRuns(allRuns);
      setTestCases(casesRes.data);
      setAutomationScripts(scriptsRes.data);
      setDefects(defectRows);
      setResults(localResults);
      // Manual step states are loaded per-run from the backend (see effect below).
      // Drop any legacy localStorage state so the UI never shows stale offline data.
      setManualStepStates({});
      if (typeof window !== "undefined") {
        try {
          window.localStorage.removeItem(`stlc_manual_step_states_project_${selectedProject}`);
        } catch {}
      }
  ...
  // When a real manual run is selected, fetch its per-step state from the backend.
  useEffect(() => {
    if (!selectedRunId || selectedRunId < 0) return;
    const run = catalogRuns.find((r) => r.id === selectedRunId);
    if (!run || modeForRun(run) !== "manual") return;
    let cancelled = false;
    executionApi
      .getManualRun(selectedRunId)
      .then((response) => {
        if (cancelled) return;
        setManualStepStates((prev) => applyManualRunDetailToStates(response.data, prev));
      })
      .catch((loadError) => {
        if (!cancelled) {
          setError(messageFromError(loadError, "Could not load manual run details."));
        }
      });
    return () => {
      cancelled = true;
    };
  }, [selectedRunId, catalogRuns]);

  // Poll an in-flight automation run until it reaches a terminal status.
  // Refreshes the run row + per-test results every 3 seconds.
  useEffect(() => {
    if (!selectedRunId || selectedRunId < 0) return;
    const run = catalogRuns.find((r) => r.id === selectedRunId);
    if (!run) return;
    const status = (run.status || "").toLowerCase();
    if (!["pending", "queued", "running", "in_progress"].includes(status)) return;
    let cancelled = false;
    const timer = setInterval(async () => {
      if (cancelled) return;
      try {
        const [runRes, resultsRes] = await Promise.all([
          executionApi.getRun(selectedRunId),
          executionApi.getResults(selectedRunId),
        ]);
        if (cancelled) return;
        setRuns((prev) => prev.map((r) => (r.id === runRes.data.id ? runRes.data : r)));
        setResults(resultsRes.data);
        const next = (runRes.data.status || "").toLowerCase();
        if (!["pending", "queued", "running", "in_progress"].includes(next)) {
          clearInterval(timer);
        }
      } catch {
        // Best-effort polling; transient errors are swallowed.
      }
    }, 3000);
    return () => {
      cancelled = true;
  ...
  const selectedResults = useMemo(() => {
    ...
    const adapterResults = typeof selectedRun.metadata_?.automation_script_id === "number"
      ? createScriptAdapterResults(selectedRun, automationScripts, testCases)
      : createWorkspaceAdapterResults(selectedRun, testCases);
  ...
  }, [automationScripts, results, selectedRun, testCases]);
  ...
  const undecidedFailuresCount = useMemo(() => {
    return selectedResults.filter(
      (result) =>
        ["failed", "error"].includes(normalizeResultStatus(result.status)) &&
  ...
  // Per-step debounce timers for text-field PATCHes. Status changes flush immediately.
  const manualPatchTimers = useMemo(() => new Map<number, ReturnType<typeof setTimeout>>(), []);
  useEffect(() => () => { manualPatchTimers.forEach((t) => clearTimeout(t)); }, [manualPatchTimers]);

  function flushManualPatch(stepId: number, payload: { status?: ManualStepStatus; actual_result?: string; comments?: string }) {
    const existing = manualPatchTimers.get(stepId);
    if (existing) {
      clearTimeout(existing);
      manualPatchTimers.delete(stepId);
    }
    executionApi
      .updateManualStep(stepId, payload)
      .then((response) => {
        setManualStepStates((prev) => {
          // Find the key that points at this stepId so we can refresh it without
          // mid-typing race conditions overwriting the user's cursor.
          const key = Object.keys(prev).find((k) => prev[k].stepId === stepId);
          if (!key) return prev;
          const merged: ManualStepState = {
            ...prev[key],
            status: response.data.status,
            // Don't clobber user's in-flight text edits — only update fields the
            // server has authoritatively. We keep local text unless server gives
            // us a different value (e.g. server applied a sanitizer).
            actualResult: response.data.actual_result ?? prev[key].actualResult,
            comments: response.data.comments ?? prev[key].comments,
            evidence: response.data.evidence ?? prev[key].evidence,
      ...
      .catch((patchError) => {
        setError(messageFromError(patchError, "Could not save manual step update."));
      });
  }
  ...
  function handleUpdateManualStep(result: ExecutionResult, stepNumber: number, patch: Partial<ManualStepState>, message?: string) {
    ...
    setManualStepStates((previous) => ({ ...previous, [key]: nextState }));

    const stepId = nextState.stepId;
    if (stepId) {
      const payload: { status?: ManualStepStatus; actual_result?: string; comments?: string } = {};
      ...
      if (Object.keys(payload).length > 0) {
        const isTextOnly = payload.status === undefined && (payload.actual_result !== undefined || payload.comments !== undefined);
        if (isTextOnly) {
          const existingTimer = manualPatchTimers.get(stepId);
          if (existingTimer) clearTimeout(existingTimer);
          const timer = setTimeout(() => flushManualPatch(stepId, payload), 600);
          manualPatchTimers.set(stepId, timer);
        } else {
          flushManualPatch(stepId, payload);
        }
      }
    }
  ...
  async function handleAttachManualEvidence(result: ExecutionResult, stepNumber: number, file: File) {
    const key = manualStepKey(result.id, stepNumber);
    const state = manualStepStates[key];
    if (!state?.stepId) {
      setError("Start the manual run before attaching evidence.");
      return;
    }
    try {
      const response = await executionApi.uploadManualEvidence(state.stepId, file);
      setManualStepStates((prev) => ({
        ...prev,
        [key]: {
          ...(prev[key] || blankManualStep()),
      ...
      pushManualActivity({
        resultId: result.id,
        stepNumber,
        message: `Evidence attached for Step ${stepNumber}: ${file.name}`,
        tone: "info",
      });
    } catch (uploadError) {
      setError(messageFromError(uploadError, "Could not upload evidence."));
    }
  }

  async function handleRemoveManualEvidence(evidenceId: string) {
    try {
      const response = await executionApi.deleteManualEvidence(evidenceId);
      setManualStepStates((prev) => {
        const key = Object.keys(prev).find((k) => prev[k].stepId === response.data.id);
        if (!key) return prev;
        return {
          ...prev,
          [key]: {
      ...
      setError(messageFromError(removeError, "Could not remove evidence."));
    }
  }
  ...
  async function handleMarkManualComplete(_result: ExecutionResult, _steps: ReturnType<typeof stepList>) {
    if (!selectedRunId || selectedRunId < 0) {
      setError("Start a manual run on the backend before marking it complete.");
      return;
    }
    try {
      const response = await executionApi.completeManualRun(selectedRunId);
      setManualStepStates((prev) => applyManualRunDetailToStates(response.data, prev));
      // Refresh run/result lists so the catalog reflects the new totals
      await loadData();
      setAutoSaveMessage(`Marked complete ${dateLabel(new Date().toISOString())}`);
      pushManualActivity({
        message: "Manual run completed",
        tone: "success",
      });
    } catch (completeError) {
      setError(messageFromError(completeError, "Could not complete manual run."));
    }
  }

  async function handleStartManualRun(adapterRun: ExecutionRun) {
    if (!selectedProject) return;
    setStartingManualRun(true);
    setError(null);
    try {
      const testCaseIds = adapterCasesForTab("manual", testCases).map((tc) => tc.id);
      if (testCaseIds.length === 0) {
        setError("No approved manual test cases to execute. Approve test cases first.");
        return;
      }
      const response = await executionApi.startManualRun(
        selectedProject,
        testCaseIds,
        adapterRun.environment || "staging",
      );
      const newRun = response.data.run;
      // Pull adapter out, drop in the real run, select it
      setRuns((prev) => [newRun, ...prev.filter((r) => r.id !== adapterRun.id)]);
      setManualStepStates((prev) => applyManualRunDetailToStates(response.data, prev));
      setSelectedRunId(newRun.id);
      pushManualActivity({
        message: `Manual run ${newRun.execution_id} started`,
        tone: "info",
      });
    } catch (startError) {
      setError(messageFromError(startError, "Could not start manual run."));
    } finally {
  ...
  function persistAdapterRuns(next: ExecutionRun[]) {
    if (typeof window === "undefined") return;
    const adapterRuns = next.filter((row) => row.id < 0);
    window.localStorage.setItem(`stlc_runs_project_${selectedProject}`, JSON.stringify(adapterRuns));
  }

  function syncAutomationRunRow(run: ExecutionRun, patch: { status: ExecutionRun["status"]; message?: string; startedAt?: string }) {
    const now = new Date().toISOString();
    setRuns((previous) => {
      const next = previous.map((row) => {
        if (row.id !== run.id) return row;
        const existingLogs = Array.isArray(row.execution_logs) ? row.execution_logs.map((entry) => String(entry)) : [];
        const nextLogs = patch.message ? [patch.message, ...existingLogs].slice(0, 8) : existingLogs;
        return {
          ...row,
          status: patch.status,
          started_at: patch.startedAt ?? row.started_at,
          updated_at: now,
          execution_logs: nextLogs,
        };
      });
      persistAdapterRuns(next);
      return next;
    });
  }

  function syncAutomationRunResults(run: ExecutionRun, patch: { status: ExecutionResult["status"]; automationStatus?: ExecutionResult["automation_execution_status"]; message?: string }) {
    const now = new Date().toISOString();
    const projectedResults = createScriptAdapterResults(run, automationScripts, testCases);
    if (projectedResults.length === 0) return;
    setResults((previous) => {
      const existingForRun = previous.filter((row) => row.execution_run_id === run.id);
      const otherResults = previous.filter((row) => row.execution_run_id !== run.id);
      const sourceRows = existingForRun.length > 0 ? existingForRun : projectedResults;
      const nextForRun = sourceRows.map((row) => ({
        ...row,
        status: patch.status,
        automation_execution_status: patch.automationStatus ?? row.automation_execution_status,
        logs: patch.message ? [patch.message, ...(row.logs || [])].slice(0, 8) : (row.logs || []),
        updated_at: now,
      }));
      const next = [...otherResults, ...nextForRun];
      if (typeof window !== "undefined") {
        window.localStorage.setItem(`stlc_results_project_${selectedProject}`, JSON.stringify(next));
      }
      return next;
  ...
  function finalizeSyntheticAutomationRun(run: ExecutionRun) {
    const outcome = deterministicAutomationOutcome(run);
    const now = new Date().toISOString();
    const statusMessage = outcome === "passed"
      ? `${run.execution_id} completed in the local automation fallback path and finished as Passed.`
      : `${run.execution_id} completed in the local automation fallback path and finished as Failed.`;
    ...
    setRuns((previous) => {
      const next = previous.map((row) => {
        ...
        return {
          ...
          skipped: 0,
          completed_at: now,
          duration_seconds: row.duration_seconds || 6,
          updated_at: now,
          execution_logs: [statusMessage, ...(Array.isArray(row.execution_logs) ? row.execution_logs.map((entry) => String(entry)) : [])].slice(0, 8),
        };
      });
      persistAdapterRuns(next);
      return next;
    ...
    setResults((previous) => {
      const projected = createScriptAdapterResults(run, automationScripts, testCases);
      const existingForRun = previous.filter((row) => row.execution_run_id === run.id);
      const otherResults = previous.filter((row) => row.execution_run_id !== run.id);
      const baseRows = existingForRun.length > 0 ? existingForRun : projected;
      const nextForRun = baseRows.map((row) => ({
        ...row,
        status: outcome,
        automation_execution_status: outcome,
        duration_ms: row.duration_ms || 6200,
        error_message: outcome === "failed" ? "AssertionError: Local automation fallback detected an execution mismatch." : undefined,
        stack_trace: outcome === "failed" ? "at syntheticRunner.execute (local://automation-fallback:1)\nat syntheticRunner.complete (local://automation-fallback:2)" : undefined,
        logs: [statusMessage, ...(row.logs || [])].slice(0, 8),
        updated_at: now,
      }));
      const next = [...otherResults, ...nextForRun];
      if (typeof window !== "undefined") {
    ...
    updateAutomationState(run, {
      ...
      currentStep: outcome === "passed" ? "Execution completed" : "Execution failed",
    });

    return outcome;
  }

  async function handoffApprovedScriptToBackend(run: ExecutionRun) {
    if (!selectedProject) return null;
    const scriptId = typeof run.metadata_?.automation_script_id === "number" ? run.metadata_.automation_script_id : null;
    if (!scriptId) return null;
    const script = automationScripts.find((row) => row.id === scriptId);
    if (!script?.test_case_id) {
      throw new Error("Approved script is not linked to a test case, so it cannot be executed yet.");
    }

    const response = await executionApi.runTests(
      selectedProject,
      [script.test_case_id],
      normalizeExecutionEnvironment(run.environment),
      run.suite_name || `${script.script_id} Automation Execution`,
      "automation",
    );

    const agentRunId = response.data?.agent_run_id;
    if (!agentRunId) {
      await loadData();
      return null;
    }
    ...
    for (let attempt = 0; attempt < 10; attempt += 1) {
      await wait(1500);
      const runsResponse = await executionApi.listRuns(selectedProject);
      const backendRun = runsResponse.data.find((row) => row.agent_run_id === agentRunId);
      if (backendRun) {
        await loadData();
        setSelectedRunId(backendRun.id);
        return backendRun;
      }
    }
  ...
  async function handleAutomationAction(run: ExecutionRun, action: "start" | "pause" | "retry_failed" | "analyze_failure" | "create_defect") {
    ...
    setError(null);
    const runResults = results.filter((result) => result.execution_run_id === run.id);
    const failedResults = runResults.filter((result) => ["failed", "error"].includes(normalizeResultStatus(result.automation_execution_status || result.status)));
    const failure = automationFailureInsight(runResults);

    try {
      if (action === "start") {
        const scriptId = typeof run.metadata_?.automation_script_id === "number"
          ? run.metadata_.automation_script_id
          : null;
        if (scriptId == null) {
          throw new Error("This automation run has no linked AutomationScript id, so the local runner cannot start it.");
        }
        updateAutomationState(run, {
          status: "queued",
          message: "Queuing on local subprocess runner...",
          currentStep: "Queuing on runner",
          startedAt: new Date().toISOString(),
        });
        const execResponse = await automationApi.executeScript(scriptId, {
          environment: run.environment || "staging",
        });
        const newRunId = execResponse.data.execution_run_id;
        // Replace the synthetic adapter run with the real one and select it
        // so the polling effect picks it up.
        setRuns((prev) => prev.filter((r) => r.id !== run.id));
        setSelectedRunId(newRunId);
        // The polling effect added below will refresh the run until terminal.
        setNotice(`${execResponse.data.message} Watching for completion.`);
      } else if (action === "pause") {
        const response = await automationExecutionService.pauseAutomationRun(run);
        updateAutomationState(run, response);
        syncAutomationRunRow(run, { status: "aborted", message: response.message });
        syncAutomationRunResults(run, { status: "blocked", automationStatus: "blocked", message: response.message });
      } else if (action === "retry_failed") {
        const response = await automationExecutionService.retryFailedTests(run, failedResults.length);
        updateAutomationState(run, response);
        syncAutomationRunRow(run, { status: response.status === "queued" ? "queued" : "failed", message: response.message });
        if (response.status === "queued") {
          syncAutomationRunResults(run, { status: "not_run", automationStatus: "pending", message: response.message });
        }
  ...
  async function queueAiExecution(testCase?: TestCase, run?: ExecutionRun) {
    if (!selectedProject || !testCase?.id) {
      setError("Select an approved AI test case before starting AI execution.");
      return;
    }

    setError(null);
    const environment = normalizeExecutionEnvironment(run?.environment || "staging");
    try {
      const response = await executionApi.runTests(
        selectedProject,
        [testCase.id],
        environment,
        `${testCase.test_case_id || "AI"} AI Execution`,
        "ai",
      );
      const agentRunId = response.data?.agent_run_id;
      let backendRun: ExecutionRun | undefined;
      let agentStatus = agentRunId ? "queued" : "completed";

      if (response.data?.run_id) {
        backendRun = (await executionApi.getRun(response.data.run_id)).data;
      } else if (agentRunId) {
        for (let attempt = 0; attempt < 20; attempt += 1) {
          await wait(1500);
          const [agentResponse, runsResponse] = await Promise.all([
            agentRunsApi.get(agentRunId),
            executionApi.listRuns(selectedProject),
          ]);
          agentStatus = agentResponse.data.status;
          backendRun = runsResponse.data.find((row) => row.agent_run_id === agentRunId);
          if (backendRun || ["failed", "cancelled"].includes(agentStatus.toLowerCase())) break;
        }
      }

      await loadData();

      if (!backendRun && agentRunId) {
        const runsResponse = await executionApi.listRuns(selectedProject);
        backendRun = runsResponse.data.find((row) => row.agent_run_id === agentRunId);
      }
      ...
      if (backendRun) {
        setSelectedRunId(backendRun.id);
        const nextStatus = aiStatusFromExecutionRun(backendRun);
        updateAiState(backendRun, {
          status: nextStatus,
          planGenerated: true,
          approved: true,
          confidence: safePercent(backendRun.passed || 0, backendRun.total_tests || 0),
          observations: [
            `${backendRun.execution_id} completed with ${backendRun.passed || 0} passed, ${backendRun.failed || 0} failed, and ${backendRun.skipped || 0} skipped.`,
            ...(run ? aiWorkspaceStateFor(run, aiWorkspaceStates[aiStateKey(run.id)]).observations : []),
          ].slice(0, 8),
        });
        setNotice(`${backendRun.execution_id} completed with ${backendRun.passed || 0} passed, ${backendRun.failed || 0} failed, and ${backendRun.skipped || 0} skipped.`);
      } else {
        setError(`AI execution was queued but no persisted execution run was returned yet. Agent status: ${normalizeStatus(agentStatus)}.`);
      }
    } catch (executionError) {
      setError(messageFromError(executionError, "AI execution could not be completed."));
      if (run) {
        const current = aiWorkspaceStateFor(run, aiWorkspaceStates[aiStateKey(run.id)]);
        updateAiState(run, {
          status: "failed",
          observations: [
            messageFromError(executionError, "AI execution failed before an execution result was persisted."),
            ...current.observations,
          ].slice(0, 8),
        });
      }
  ...
  async function handleRunAiExecution(run: ExecutionRun) {
    const current = aiWorkspaceStateFor(run, aiWorkspaceStates[aiStateKey(run.id)]);
    if (!current.planGenerated || !current.approved) return;
    if (!selectedProject || !selectedTestCase?.id) {
      updateAiState(run, {
        status: "needs_review",
        observations: [
          "AI execution could not start because no executable test case is selected.",
          ...current.observations,
        ].slice(0, 8),
      });
      return;
    ...
    updateAiState(run, {
      status: "in_progress",
      observations: [
        "AI execution queued in review-gated mode through the backend execution agent.",
        ...current.observations,
      ].slice(0, 8),
    });
    await queueAiExecution(selectedTestCase, run);
  }
    ...
    <div className="space-y-5 pb-8">
      ...
      <main className="flex flex-col gap-6">
        <WorkspaceShell
          ...
          onRemoveManualEvidence={handleRemoveManualEvidence}
          onCreateManualDefect={handleCreateManualDefect}
          onStartManualRun={handleStartManualRun}
          startingManualRun={startingManualRun}
          creatingDefectKey={creatingDefectKey}
          manualActivity={manualActivity}
          autoSaveMessage={autoSaveMessage}
          ...
          onRunAiTestCase={queueAiExecution}
