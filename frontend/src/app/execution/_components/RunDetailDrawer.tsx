    ...
    <div className="rounded-lg border border-gray-200">
      ...
      {expanded && (
        <div className="space-y-3 border-t border-gray-100 bg-gray-50/50 p-3">
          ...
          {result.stack_trace && (
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Stack trace</p>
              <pre className="mt-1 max-h-48 overflow-auto whitespace-pre-wrap rounded-md bg-gray-900 p-2 font-mono text-[11px] leading-relaxed text-gray-100">
                {result.stack_trace}
              </pre>
            </div>
          )}
          {result.logs && result.logs.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Test log</p>
              <pre className="mt-1 max-h-40 overflow-auto whitespace-pre-wrap rounded-md bg-gray-900 p-2 font-mono text-[11px] leading-relaxed text-gray-100">
          ...
          {screenshotSrc && (
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Screenshot</p>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={screenshotSrc}
                alt={`Screenshot for ${result.test_name}`}
                className="mt-1 max-h-64 rounded-md border border-gray-200 object-contain"
                loading="lazy"
              />
            </div>
          )}
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent size="xl">
        <DrawerHeader>
            ...
            <div className="flex items-center gap-2">
              ...
              {runId !== null && (
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-md border border-gray-200 px-2 py-0.5 text-[10px] font-semibold text-gray-600 hover:bg-gray-50"
          ...
          <DrawerClose className="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
        ...
        <DrawerBody>
          ...
          {run && (
            <>
              ...
              {progressPct !== null && (
                <div className="rounded-lg border border-app-brand-100 bg-app-brand-75/50 px-4 py-3">
                  <div className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-app-brand-700">
                    <span>Run in progress</span>
                    <span className="tabular-nums">{progressPct}%</span>
                  </div>
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-app-brand-100">
                    <div
                      className="h-full rounded-full bg-[#B71920] transition-all"
              ...
              {/* Pass/fail strip */}
              <div className="flex items-center gap-4 rounded-lg border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm">
                <span className="font-semibold text-gray-700">{run.total_tests} tests</span>
              ...
              {executionLogs.length > 0 && (
                <section>
                  ...
                  <pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded-lg bg-gray-900 p-3 font-mono text-[11px] leading-relaxed text-gray-100">
                    {executionLogs.join("\n")}
                  </pre>
                </section>
              )}
