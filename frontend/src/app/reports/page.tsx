...
function ProgressBar({ value, max, color = "bg-[#B71920]" }: { value: number; max: number; color?: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-bold text-gray-500 w-8 text-right shrink-0">{pct}%</span>
    </div>
  );
}
...
function ReportRow({ report }: { report: Report }) {
  ...
  return (
    <div className={cn(
      "bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-200 hover:border-gray-350",
      expanded && "ring-1 ring-[#B71920]/10"
    )}>
      <div
        className="p-4 flex items-start justify-between gap-3 cursor-pointer hover:bg-gray-50/50"
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            <span className="text-[10px] font-mono font-bold text-gray-400">{report.report_id}</span>
            <Badge variant={getStatusVariant(report.report_type)} className="uppercase py-0 text-[9px]">
              {report.report_type}
            </Badge>
            <span className="text-[10px] text-gray-400 font-bold">{new Date(report.created_at).toLocaleString()}</span>
          </div>
          <h4 className="text-xs font-bold text-gray-800 leading-snug">{report.title}</h4>
          {report.summary && !expanded && (
            <p className="text-[11px] text-gray-500 mt-1 line-clamp-1">{report.summary}</p>
          )}
        </div>
        ...
        <div className="border-t border-gray-100 p-5 space-y-5 bg-gray-50/30">
          ...
          {report.summary && (
            <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-1.5 shadow-sm">
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Executive Summary</label>
              <p className="text-xs text-gray-700 leading-relaxed font-semibold">{report.summary}</p>
            </div>
          )}

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {/* Coverage */}
            <GridSection title="Test Coverage Summary" icon={Shield} iconColor="text-app-brand-500">
              {cov.requirements_total !== undefined && (
                <div>
                  <div className="flex justify-between text-[10px] font-semibold text-gray-500 mb-1">
                    ...
                    <span className="font-bold text-gray-700">{cov.requirements_approved ?? 0} / {cov.requirements_total ?? 0}</span>
                  </div>
                  <ProgressBar value={Number(cov.requirements_approved ?? 0)} max={Number(cov.requirements_total ?? 1)} color="bg-app-brand-500" />
                </div>
              )}
              {cov.test_cases_total !== undefined && (
                <div>
                  <div className="flex justify-between text-[10px] font-semibold text-gray-500 mb-1">
                    ...
                    <span className="font-bold text-gray-700">{cov.test_cases_approved ?? 0} / {cov.test_cases_total ?? 0}</span>
                  </div>
              ...
              {cov.automation_coverage_pct !== undefined && (
                <div>
                  <div className="flex justify-between text-[10px] font-semibold text-gray-500 mb-1">
                    ...
                    <span className="font-bold text-gray-700">{cov.automation_coverage_pct}%</span>
                  </div>
            ...
            <GridSection title="Execution Metrics Pipeline" icon={TrendingUp} iconColor="text-emerald-500">
              <div className="space-y-2 text-xs font-semibold">
                {Object.entries(exec).map(([k, v]) => (
                  <div key={k} className="flex justify-between border-b border-gray-50 pb-1.5 last:border-b-0 last:pb-0">
                    <span className="text-[10px] text-gray-500 capitalize">{k.replace(/_/g, " ")}</span>
                    <span className="text-gray-800 font-bold">{String(v)}</span>
                  </div>
                ))}
              </div>
            </GridSection>
    ...
    <div className="space-y-6 select-none pb-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="rounded-xl bg-app-brand-75 border border-app-brand-100 p-2.5">
            <BarChart3 className="h-6 w-6 text-[#B71920]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Reports & Analytics</h1>
            <p className="text-xs text-gray-500 mt-1">Compile sprint quality metrics, analyse risks, and view coverage health trends (Agent 11)</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <select
            ...
            className="appearance-none bg-white hover:bg-gray-50 border border-gray-200 text-gray-800 rounded-lg text-xs font-semibold px-3 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-[#B71920] transition-colors cursor-pointer"
            style={{
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-250">
          ...
          <p className="text-xs text-gray-500 font-bold">No reports generated yet</p>
          <p className="text-[10px] text-gray-400 font-semibold mt-1">Select project and type above, then click Generate Report to run AI analysis</p>
        </div>
      ) : (
