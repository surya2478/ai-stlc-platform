...
function Progress({ value, tone = "emerald" }: { value: number; tone?: Tone }) {
  return <div className="h-1.5 w-full rounded-full bg-gray-100"><div className={cn("h-full rounded-full", tone === "emerald" ? "bg-emerald-500" : tone === "amber" ? "bg-amber-500" : tone === "red" ? "bg-red-500" : "bg-app-brand-500")} style={{ width: `${Math.max(0, Math.min(100, value))}%` }} /></div>;
}

function Kpi({ label, value, detail, icon: Icon, tone }: { label: string; value: number; detail: string; icon: typeof FileText; tone: Tone }) {
  return <div className="rounded-lg border border-gray-200 bg-white px-3 py-3 shadow-sm">
    <div className="flex items-center gap-2"><span className={cn("flex h-7 w-7 items-center justify-center rounded-md", toneClass(tone))}><Icon className="h-3.5 w-3.5" /></span><p className="text-[10px] font-extrabold text-gray-800">{label}</p></div>
    <p className="mt-3 text-xl font-black leading-none text-gray-950">{value}</p><p className="mt-2 text-[9px] font-semibold text-gray-500">{detail}</p>
  </div>;
}

function Empty({ children }: { children: React.ReactNode }) {
  return <div className="rounded-md border border-dashed border-gray-200 p-3 text-[10px] font-semibold text-gray-500">{children}</div>;
}

function Panel({ title, action, children, className }: { title: string; action?: React.ReactNode; children: React.ReactNode; className?: string }) {
  return <section className={cn("rounded-lg border border-gray-200 bg-white p-3", className)}><div className="mb-2 flex items-center justify-between"><h3 className="text-[10px] font-extrabold text-gray-800">{title}</h3>{action}</div>{children}</section>;
}
...
export function TestCaseApprovalView({ projectId, initialTestCaseId = null }: { projectId: number | null; initialTestCaseId?: number | null }) {
  ...
  return <div className="min-h-full pb-3">
    <main className="space-y-3">
      ...
      <section className="rounded-lg border border-gray-200 bg-white px-3 py-3 shadow-sm"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-[10px] font-extrabold uppercase tracking-wide text-gray-700">Approval & Downstream Readiness</h2><p className="mt-1 text-[9px] font-semibold text-gray-500">Only approval-stage checks block the reviewer decision. Blue clock items are completed later.</p></div><span className="text-[9px] font-bold text-[#B71920]">Persisted project records</span></div><div className="grid grid-cols-4 gap-2 2xl:grid-cols-9">
        {["requirement", "scenario", "validation", "journey", "application", "evidence", "discovery", "policy", "automation_classification"].map((key) => { const item = readiness(key); const label = rows[0]?.checks.find((check) => check.key === key)?.label || key; const later = item.scope === "downstream" && item.warnings > 0; return <div key={key} className="flex min-w-0 gap-2"><span className={cn("mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full", item.good ? "bg-emerald-50 text-emerald-600" : later ? "bg-app-brand-75 text-app-brand-600" : "bg-red-50 text-red-600")}>{item.good ? <CheckCircle2 className="h-3.5 w-3.5" /> : later ? <Clock3 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />}</span><div className="min-w-0"><p className="text-[8px] font-bold leading-3 text-gray-700">{label}</p><p className="mt-1 text-[9px] font-extrabold text-gray-900">{item.passed}/{item.total}{later ? " · Later stage" : ""}</p></div></div>; })}
      </div></section>
      ...
      <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        ] as Array<[QueueTab, string]>).map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={cn("rounded-md px-3 py-1.5 text-[9px] font-extrabold", tab === key ? "bg-[#B71920] text-white" : "text-gray-600 hover:bg-gray-50")}>{label}<span className={cn("ml-1 rounded px-1 py-0.5", tab === key ? "bg-white/20" : "bg-gray-100")}>{counts[key]}</span></button>)}</div>
        <div className="space-y-2 border-b border-gray-100 p-3">
          <div className="grid grid-cols-[minmax(230px,1.6fr)_repeat(5,minmax(110px,1fr))_94px] gap-2"><label className="relative"><Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" /><input aria-label="Search approval queue" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by TC ID, requirement ID, PPM ID, title, scenario or journey" className="h-8 w-full rounded-md border border-gray-200 pl-9 pr-3 text-[9px] outline-none focus:border-app-brand-500" /></label><FilterSelect label="Review status" value={reviewFilter} setValue={setReviewFilter} options={Object.values({ ready: "Ready", pending: "Pending Review", changes: "Changes Requested", approved: "Approved", rejected: "Rejected", blocked: "Blocked" })} /><FilterSelect label="Domain" value={domainFilter} setValue={setDomainFilter} options={domains} /><FilterSelect label="Test type" value={typeFilter} setValue={setTypeFilter} options={testTypes} /><FilterSelect label="Scenario class" value={classFilter} setValue={setClassFilter} options={classes} /><FilterSelect label="Priority" value={priorityFilter} setValue={setPriorityFilter} options={priorities} /><Button variant="outline" onClick={() => setShowMoreFilters(!showMoreFilters)} className="h-8 gap-1 text-[9px] font-bold"><Filter className="h-3.5 w-3.5" />More Filters</Button></div>
          {showMoreFilters && <div className="grid grid-cols-[repeat(4,minmax(150px,1fr))_94px] gap-2"><FilterSelect label="Assigned reviewer" value={reviewerFilter} setValue={setReviewerFilter} options={activeReviewerUsers.map((user) => user.full_name)} optionValues={activeReviewerUsers.map((user) => String(user.id))} extra={[{ value: "unassigned", label: "Unassigned" }]} /><FilterSelect label="Journey readiness" value={journeyFilter} setValue={setJourneyFilter} options={["Ready", "Missing / Unreviewed"]} optionValues={["ready", "missing"]} /><FilterSelect label="Evidence status" value={evidenceFilter} setValue={setEvidenceFilter} options={["Complete", "Missing"]} optionValues={["complete", "missing"]} /><FilterSelect label="Application mapping" value={applicationFilter} setValue={setApplicationFilter} options={["Mapped", "Missing"]} optionValues={["mapped", "missing"]} /><button onClick={clearFilters} className="h-8 text-[9px] font-extrabold text-[#B71920]">Clear Filters</button></div>}
        </div>

        <div className="overflow-x-auto"><div className="min-w-[1250px]"><div className="grid items-center border-b border-gray-200 bg-gray-50 px-2 py-2 text-[8px] font-extrabold uppercase tracking-wide text-gray-500" style={{ gridTemplateColumns: GRID }}><span><input aria-label="Select visible test cases" type="checkbox" checked={pageRows.length > 0 && pageRows.every((row) => selectedIds.has(row.testCase.id))} onChange={(event) => setSelectedIds((current) => { const next = new Set(current); pageRows.forEach((row) => event.target.checked ? next.add(row.testCase.id) : next.delete(row.testCase.id)); return next; })} /></span><span>TC ID</span><span>Requirement<br />ID / PPM ID</span><span>Title</span><span>Test type</span><span>Scenario class</span><span>Priority</span><span>Journey coverage</span><span>Validation</span><span>Evidence</span><span>Review status</span><span>Reviewer</span><span>SLA / age</span><span>Updated</span><span /></div>
          {loading ? <div className="flex h-40 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-app-brand-600" /></div> : pageRows.length ? pageRows.map((row) => <div key={row.testCase.id} onClick={() => { setSelectedId(row.testCase.id); setInspectorOpen(true); }} className={cn("relative grid min-h-[54px] cursor-pointer items-center border-b border-gray-100 px-2 py-2 text-[9px] font-semibold text-gray-600 hover:bg-app-brand-75/40", selected?.testCase.id === row.testCase.id && "bg-app-brand-75/50 ring-1 ring-inset ring-app-brand-500")} style={{ gridTemplateColumns: GRID }}><span onClick={(event) => event.stopPropagation()}><input aria-label={`Select ${row.testCase.test_case_id}`} type="checkbox" checked={selectedIds.has(row.testCase.id)} onChange={(event) => setSelectedIds((current) => { const next = new Set(current); event.target.checked ? next.add(row.testCase.id) : next.delete(row.testCase.id); return next; })} /></span><button className="text-left font-extrabold text-[#B71920]">{row.testCase.test_case_id}</button><span><strong className="block text-gray-800">{row.requirement?.requirement_id || "Unlinked"}</strong><span className="text-[8px] text-gray-500">{ppmId(row.requirement)}</span></span><span className="pr-2 font-bold leading-3 text-gray-800">{row.testCase.title}</span><span className="text-app-brand-700">{row.testCase.test_type || "Unassigned"}</span><span className="truncate pr-1">{scenarioClass(row.scenario, row.testCase)}</span><span><Badge tone={normal(row.testCase.priority).includes("high") ? "red" : normal(row.testCase.priority).includes("medium") ? "amber" : "emerald"}>{row.testCase.priority}</Badge></span><span className="pr-2"><span className="mb-1 block font-bold text-gray-800">{row.journeyCoverage}%</span><Progress value={row.journeyCoverage} tone={row.journeyCoverage >= 80 ? "emerald" : "amber"} /></span><span><span className={cn("flex h-7 w-7 items-center justify-center rounded-full border text-[9px] font-black", row.validationScore === 100 ? "border-emerald-300 text-emerald-700" : "border-amber-300 text-amber-700")}>{row.validationScore}</span></span><span><Badge tone={row.evidence.length ? "emerald" : "red"}>{row.evidence.length ? "Complete" : "Missing"}</Badge></span><span><Badge tone={statusTone(row.status)}>{row.status}</Badge></span><span className="flex items-center gap-1"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-app-brand-600 text-[7px] font-black text-white">{row.reviewer ? row.reviewer.full_name.split(" ").map((part) => part[0]).join("").slice(0, 2) : "—"}</span><span className="truncate">{row.reviewer?.full_name || "Unassigned"}</span></span><span className={cn("font-bold", ageLabel(row.testCase.updated_at).startsWith("0") ? "text-emerald-600" : "text-red-600")}>{ageLabel(row.testCase.updated_at)}</span><span>{shortDate(row.testCase.updated_at)}</span><span className="relative" onClick={(event) => event.stopPropagation()}><button aria-label={`Actions for ${row.testCase.test_case_id}`} onClick={() => setActionMenu(actionMenu === row.testCase.id ? null : row.testCase.id)} className="flex h-7 w-7 items-center justify-center rounded border border-gray-200"><MoreVertical className="h-3.5 w-3.5" /></button>{actionMenu === row.testCase.id && <div className="absolute right-0 top-8 z-30 w-36 rounded-md border border-gray-200 bg-white p-1 shadow-xl"><button onClick={() => { setSelectedId(row.testCase.id); setInspectorOpen(true); setActionMenu(null); }} className="w-full rounded px-2 py-1.5 text-left text-[9px] font-bold hover:bg-gray-50">Open review</button><button onClick={() => router.push(`/test-cases?project=${projectId}&view=editor&case=${row.testCase.id}`)} className="w-full rounded px-2 py-1.5 text-left text-[9px] font-bold hover:bg-gray-50">Open editor</button></div>}</span></div>) : <div className="p-8 text-center text-[10px] font-semibold text-gray-500">No test cases match the selected filters.</div>}
        </div></div>
        <div className="flex items-center justify-between px-3 py-2 text-[9px] font-semibold text-gray-500"><span>Showing {filtered.length ? (page - 1) * PAGE_SIZE + 1 : 0} to {Math.min(page * PAGE_SIZE, filtered.length)} of {filtered.length} test cases</span><div className="flex items-center gap-1"><button aria-label="Previous page" disabled={page === 1} onClick={() => setPage((value) => Math.max(1, value - 1))} className="flex h-7 w-7 items-center justify-center rounded border disabled:opacity-40"><ChevronLeft className="h-3.5 w-3.5" /></button><span className="flex h-7 min-w-7 items-center justify-center rounded bg-[#B71920] px-2 font-bold text-white">{page}</span><span>of {pageCount}</span><button aria-label="Next page" disabled={page === pageCount} onClick={() => setPage((value) => Math.min(pageCount, value + 1))} className="flex h-7 w-7 items-center justify-center rounded border disabled:opacity-40"><ChevronRight className="h-3.5 w-3.5" /></button><span className="ml-2 rounded border px-2 py-1.5">{PAGE_SIZE} / page</span></div></div>
      </section>
    </main>

    <Drawer open={inspectorOpen && !!selected} onOpenChange={setInspectorOpen}>
    <DrawerContent size="lg" className="bg-gray-50/70">
      {selected && <div className="flex-1 overflow-y-auto">
        <div className="sticky top-0 z-20 border-b border-gray-200 bg-white px-3 pt-3"><div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-2"><DrawerTitle className="text-sm text-gray-900">{selected.testCase.test_case_id}</DrawerTitle><Badge tone={statusTone(selected.status)}>{selected.status}</Badge></div><p className="mt-2 text-[11px] font-extrabold text-gray-800">{selected.testCase.title}</p></div><button aria-label="Close inspector" onClick={() => setInspectorOpen(false)}><X className="h-4 w-4 text-gray-500" /></button></div><div className="mt-3 flex gap-3 overflow-x-auto">{(["review", "traceability", "test-case", "evidence", "automation", "history", "activity"] as InspectorTab[]).map((item) => <button key={item} onClick={() => setInspectorTab(item)} className={cn("border-b-2 px-0.5 pb-2 text-[9px] font-extrabold capitalize", inspectorTab === item ? "border-[#B71920] text-[#B71920]" : "border-transparent text-gray-500")}>{item.replace("-", " ")}</button>)}</div></div>
        <div className="space-y-2 p-3">
          ...
          <Panel title="Review Actions" className="border-amber-200 bg-amber-50/40">
            ...
            {selected.blockers.length === 0 && selected.checks.some((check) => check.scope === "downstream" && check.state !== "pass") && <div className="mb-2 rounded-md border border-app-brand-200 bg-app-brand-75 p-2 text-[9px] font-semibold text-app-brand-800"><strong>Ready for approval.</strong> Remaining journey, evidence, discovery, application, or automation items are tracked for later stages and do not disable this action.</div>}
            {decisionMode && <div className="mb-2 rounded-md border border-gray-200 bg-white p-2"><label className="text-[9px] font-extrabold text-gray-700">Reviewer comment (required)</label><textarea aria-label="Reviewer comment" value={comment} onChange={(event) => setComment(event.target.value)} className="mt-1 h-16 w-full resize-none rounded border border-gray-200 p-2 text-[10px]" /><div className="mt-2 flex gap-2"><Button onClick={() => void submitDecision()} disabled={!comment.trim() || Boolean(busy)} className={cn("h-8 flex-1 text-[9px] font-bold text-white", decisionMode === "reject" ? "bg-red-600" : "bg-amber-500")}>Confirm {decisionMode === "reject" ? "Rejection" : "Changes"}</Button><Button variant="outline" onClick={() => { setDecisionMode(null); setComment(""); }} className="h-8 text-[9px] font-bold">Cancel</Button></div></div>}
            {assigning && <div className="mb-2 rounded-md border border-gray-200 bg-white p-2"><select aria-label="Assigned reviewer" value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)} className="h-8 w-full rounded border border-gray-200 px-2 text-[9px]"><option value="">Select reviewer</option>{activeReviewerUsers.map((user) => <option key={user.id} value={user.id}>{user.full_name} · {activeMembershipByUser.get(user.id)?.role}</option>)}</select><div className="mt-2 flex gap-2"><Button onClick={() => void assignReviewer()} disabled={!assigneeId || Boolean(busy)} className="h-8 flex-1 bg-[#B71920] text-[9px] font-bold text-white">Save Assignment</Button><Button variant="outline" onClick={() => setAssigning(false)} className="h-8 text-[9px] font-bold">Cancel</Button></div></div>}
            <div className="grid grid-cols-1 gap-2 2xl:grid-cols-3"><Button onClick={() => void approveRows([selected])} disabled={selected.blockers.length > 0 || Boolean(busy) || !canApprove || selected.status === "Approved"} className="h-9 bg-emerald-500 text-[9px] font-bold text-white disabled:bg-gray-200"><ShieldCheck className="mr-1 h-3.5 w-3.5" />Approve Test Case</Button><Button variant="outline" onClick={() => setDecisionMode("changes")} className="h-9 border-amber-300 text-[9px] font-bold text-amber-700">Request Changes</Button><Button variant="outline" onClick={() => setDecisionMode("reject")} className="h-9 border-red-300 text-[9px] font-bold text-red-700">Reject Test Case</Button></div>
...
function FilterSelect({ label, value, setValue, options, optionValues, extra = [] }: { label: string; value: string; setValue: (value: string) => void; options: string[]; optionValues?: string[]; extra?: Array<{ value: string; label: string }> }) {
  return <select aria-label={label} value={value} onChange={(event) => setValue(event.target.value)} className="h-8 min-w-0 rounded-md border border-gray-200 bg-white px-2 text-[9px] font-semibold text-gray-600"><option value="all">{label}: All</option>{extra.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}{options.map((item, index) => <option key={`${item}-${index}`} value={optionValues?.[index] ?? item}>{item}</option>)}</select>;
}

function CheckList({ checks }: { checks: GovernanceCheck[] }) {
  return <div className="space-y-1.5">{checks.map((check) => <div key={check.key} className="flex items-start justify-between gap-2"><span className="flex min-w-0 items-start gap-2 text-[9px] font-semibold text-gray-600">{check.state === "pass" ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-emerald-500" /> : check.scope === "downstream" ? <Clock3 className="h-3.5 w-3.5 shrink-0 text-app-brand-500" /> : <AlertTriangle className={cn("h-3.5 w-3.5 shrink-0", check.state === "blocker" ? "text-red-500" : "text-amber-500")} />}<span>{check.label}{check.scope === "downstream" && check.state !== "pass" && <span className="ml-1 rounded bg-app-brand-75 px-1 py-0.5 text-[7px] font-extrabold text-app-brand-700">LATER</span>}</span></span><span className="max-w-[48%] text-right text-[8px] font-bold text-gray-500">{check.detail}</span></div>)}</div>;
}
...
function ReviewInspector({ row, history, approvals, onTrace }: { row: ApprovalRow; history: TestCaseHistory[]; approvals: ApprovalAction[]; onTrace: () => void }) {
  ...
  return <>
    <div className="grid grid-cols-4 gap-2 rounded-lg border border-gray-200 bg-white p-3 text-[9px]"><LabelValue label="Test case ID" value={row.testCase.test_case_id} /><LabelValue label="Current status" value={row.status} /><LabelValue label="Test type" value={row.testCase.test_type || "Unassigned"} /><LabelValue label="Priority" value={row.testCase.priority} /></div>
    <Panel title="What is required to approve" className={row.blockers.length ? "border-red-200 bg-red-50/40" : "border-emerald-200 bg-emerald-50/40"}>
      ...
      <p className="mt-1 text-[9px] font-semibold leading-4 text-gray-600">Approval validates test-case content, approved requirement/scenario traceability, applicable gating review, and reviewer permission. Journey, application, evidence, discovery, and automation preparation happen afterward.</p>
      {row.blockers.length > 0 && <div className="mt-3 space-y-2">{row.blockers.map((item) => <div key={item.key} className="rounded-md border border-red-100 bg-white p-2"><p className="text-[9px] font-extrabold text-red-800">{item.label}: {item.detail}</p><p className="mt-1 text-[8px] font-semibold leading-4 text-gray-600"><strong>Update in {item.owner || "the owning workspace"}:</strong> {item.guidance}</p></div>)}</div>}
      {downstreamItems.length > 0 && <div className="mt-3 rounded-md border border-app-brand-100 bg-app-brand-75 p-2"><p className="text-[9px] font-extrabold text-app-brand-800">Later-stage preparation — does not block approval</p><ul className="mt-1.5 space-y-1">{downstreamItems.map((item) => <li key={item.key} className="text-[8px] font-semibold leading-4 text-app-brand-900"><strong>{item.owner}:</strong> {item.guidance}</li>)}</ul></div>}
    </Panel>
    <div className="grid grid-cols-1 gap-2 2xl:grid-cols-3">
      <Panel title="Readiness Summary">
        <p className="text-lg font-black text-gray-900">{approvalPassed} / {approvalChecks.length}</p>
        <p className="mb-2 text-[8px] font-semibold text-gray-500">approval-stage gates satisfied</p>
      ...
      <Panel title="Deterministic Validation">
        ...
        <p className="mt-3 text-[8px] font-semibold text-gray-500">Required-field completeness</p>
        <p className="mt-1 text-xl font-black text-gray-900">{row.validationScore} / 100</p>
        <button onClick={onTrace} className="mt-4 text-[9px] font-bold text-[#B71920]">View details</button>
      </Panel>
      <Panel
      ...
      >
        {row.review ? (
          <>
            ...
            <p className="mt-3 text-[8px] font-semibold text-gray-500">Set-level score</p>
            <p className="mt-1 text-lg font-black text-purple-700">{reviewScoreLabel(row.review.overall_score)}</p>
            <p className="mt-2 text-[8px] font-semibold leading-4 text-gray-500">
    ...
    <Panel title="Reviewer, SLA & Approval Findings"><div className="grid grid-cols-3 gap-3"><LabelValue label="Assigned reviewer" value={row.reviewer?.full_name || "Unassigned"} /><LabelValue label="Review SLA / age" value={ageLabel(row.testCase.updated_at)} /><LabelValue label="Approval blockers" value={`${row.blockers.length} blocker${row.blockers.length === 1 ? "" : "s"}`} /></div>{row.blockers.length > 0 ? <ul className="mt-3 space-y-2">{row.blockers.map((item) => <li key={item.key} className="rounded-md border border-red-100 bg-red-50 p-2 text-[9px] font-semibold text-red-700"><strong>{item.label}:</strong> {item.detail}<span className="mt-1 block text-[8px] text-gray-600">{item.guidance}</span></li>)}</ul> : <p className="mt-3 rounded-md border border-emerald-100 bg-emerald-50 p-2 text-[9px] font-semibold text-emerald-700">No approval blockers. The reviewer can approve this test case now; later-stage items remain visible for downstream teams.</p>}</Panel>
    <Panel title="Traceability Snapshot" action={<button onClick={onTrace} className="text-[8px] font-bold text-[#B71920]">View full trace</button>}><div className="grid grid-cols-2 gap-2 2xl:grid-cols-6"><LabelValue label="Requirement" value={row.requirement?.requirement_id || "Unlinked"} /><LabelValue label="PPM ID" value={ppmId(row.requirement)} /><LabelValue label="Scenario" value={row.scenario?.scenario_id || "Unlinked"} /><LabelValue label="Journey" value={row.journeyId} /><LabelValue label="Application" value={row.application?.name || "Missing"} /><LabelValue label="Discovery" value={row.discovery} /></div></Panel>
    <div className="grid grid-cols-1 gap-2 2xl:grid-cols-3"><Panel title="Test Case Preview"><p className="text-[8px] font-semibold text-gray-500">Preconditions</p><p className="mt-1 text-[9px] font-bold text-gray-800">{row.testCase.preconditions?.length || 0} recorded</p><p className="mt-2 text-[8px] font-semibold text-gray-500">Test steps</p><p className="mt-1 text-[9px] font-bold text-gray-800">{row.testCase.steps?.length || 0} ordered steps</p><p className="mt-2 text-[8px] font-semibold text-gray-500">Expected result</p><p className="mt-1 line-clamp-2 text-[9px] font-semibold text-gray-700">{row.testCase.expected_result || "Missing"}</p></Panel><Panel title="Evidence Summary"><p className="text-[8px] font-semibold text-gray-500">Required evidence</p><div className="mt-2 flex flex-wrap gap-1">{row.evidence.map((item) => <Badge key={item} tone="blue">{item}</Badge>)}{!row.evidence.length && <Badge tone="red">Missing</Badge>}</div><p className="mt-3 text-[8px] font-semibold text-gray-500">Latest execution evidence</p><p className="mt-1 text-[9px] font-bold text-gray-800">{row.testCase.latest_evidence_available ? "Available" : "Not available"}</p></Panel><Panel title="History / Activity"><p className="text-[9px] font-bold text-gray-800">{history.length} editor change{history.length === 1 ? "" : "s"}</p><p className="mt-2 text-[9px] font-bold text-gray-800">{approvals.length} approval action{approvals.length === 1 ? "" : "s"}</p><p className="mt-2 text-[8px] font-semibold text-gray-500">Latest activity</p><p className="mt-1 text-[9px] font-semibold text-gray-700">{shortDate(history[0]?.created_at || approvals[0]?.created_at || row.testCase.updated_at)}</p></Panel></div>
  </>;
}

function TraceabilityInspector({ row, projectId, router }: { row: ApprovalRow; projectId: number; router: ReturnType<typeof useRouter> }) {
  return <><Panel title="Requirement"><LabelValue label="Requirement ID" value={row.requirement?.requirement_id || "Unlinked"} /><div className="mt-2"><LabelValue label="PPM ID" value={ppmId(row.requirement)} /></div><p className="mt-2 text-[9px] font-semibold text-gray-700">{row.requirement?.title || "No linked requirement title"}</p></Panel><Panel title="Scenario"><LabelValue label="Scenario ID" value={row.scenario?.scenario_id || "Unlinked"} /><div className="mt-2"><LabelValue label="Scenario class" value={scenarioClass(row.scenario, row.testCase)} /></div><p className="mt-2 text-[9px] font-semibold text-gray-700">{row.scenario?.title || "No linked scenario title"}</p></Panel><Panel title="Journey"><div className="grid grid-cols-2 gap-2"><LabelValue label="Journey ID" value={row.journeyId} /><LabelValue label="Journey name" value={row.journeyName} /></div><div className="mt-3"><Progress value={row.journeyCoverage} tone={row.journeyCoverage >= 80 ? "emerald" : "amber"} /></div><p className="mt-1 text-[8px] font-semibold text-gray-500">{row.journeyCoverage}% required scenario-class coverage</p></Panel><Panel title="Application & Discovery"><div className="grid grid-cols-2 gap-2"><LabelValue label="Application mapping" value={row.application?.name || "Missing"} /><LabelValue label="Discovery eligibility" value={row.discovery} /></div></Panel><Button variant="outline" onClick={() => router.push(`/requirements?project=${projectId}&view=traceability&requirement=${row.requirement?.id || ""}`)} disabled={!row.requirement} className="h-9 w-full gap-2 text-[9px] font-bold"><ExternalLink className="h-3.5 w-3.5" />Open Requirement Traceability</Button></>;
}

function TestCaseInspector({ row, onEdit }: { row: ApprovalRow; onEdit: () => void }) {
  return <><Panel title="Preconditions">{row.testCase.preconditions?.length ? <ol className="list-decimal space-y-1 pl-4 text-[9px] font-semibold text-gray-700">{row.testCase.preconditions.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ol> : <Empty>No preconditions recorded.</Empty>}</Panel><Panel title="Ordered Test Steps">{row.testCase.steps?.length ? <div className="overflow-hidden rounded border border-gray-200"><div className="grid grid-cols-[28px_1fr_1fr] bg-gray-50 p-2 text-[8px] font-extrabold uppercase text-gray-500"><span>#</span><span>Action</span><span>Expected result</span></div>{row.testCase.steps.map((step, index) => <div key={`${step.step_number}-${index}`} className="grid grid-cols-[28px_1fr_1fr] border-t border-gray-100 p-2 text-[9px] font-semibold text-gray-700"><span>{step.step_number}</span><span>{step.action || "Missing"}</span><span>{step.expected_result || "Missing"}</span></div>)}</div> : <Empty>No test steps recorded.</Empty>}</Panel><Panel title="Test Data & Expected Result"><pre className="max-h-28 overflow-auto whitespace-pre-wrap rounded bg-gray-50 p-2 text-[8px] text-gray-700">{row.testCase.test_data && Object.keys(row.testCase.test_data).length ? JSON.stringify(row.testCase.test_data, null, 2) : "No test data reference recorded."}</pre><p className="mt-2 text-[9px] font-semibold text-gray-700">{row.testCase.expected_result || "No overall expected result recorded."}</p></Panel><Panel title="Classification"><div className="grid grid-cols-4 gap-2"><LabelValue label="Priority" value={row.testCase.priority} /><LabelValue label="Severity" value={row.testCase.severity} /><LabelValue label="Test type" value={row.testCase.test_type || "Unassigned"} /><LabelValue label="Automation" value={row.testCase.automation_candidate ? "Candidate" : "Manual"} /></div></Panel><Panel title="Validation Findings"><CheckList checks={row.checks.filter((item) => ["validation", "requirement", "scenario"].includes(item.key))} /></Panel><Button onClick={onEdit} className="h-9 w-full bg-[#B71920] text-[9px] font-bold text-white">Open Test Case Editor</Button></>;
}
...
const CLASSIFICATION_DECISIONS: Array<{ key: "approve" | "approve_conditional" | "not_recommended" | "defer" | "request_changes"; label: string; reasonRequired: boolean; tone: string }> = [
  ...
  { key: "defer", label: "Defer", reasonRequired: true, tone: "border-gray-300 text-gray-700" },
  { key: "request_changes", label: "Request Changes", reasonRequired: true, tone: "border-app-brand-300 text-app-brand-700" },
];
...
function ClassificationInspector({ row, enabled, canEvaluate, canApprove, onChanged }: { row: ApprovalRow; enabled: boolean; canEvaluate: boolean; canReview: boolean; canApprove: boolean; onChanged: () => void }) {
  ...
  return <>
    ...
    {!classification ? <Panel title="Automation Classification"><Empty>This test case has not been classified yet.</Empty>{canEvaluate && <Button onClick={() => void runEvaluate()} disabled={Boolean(busy)} className="mt-2 h-9 w-full bg-[#B71920] text-[9px] font-bold text-white"><Bot className="mr-1 h-3.5 w-3.5" />Classify Now</Button>}</Panel> : <>
      <div className="grid grid-cols-4 gap-2 rounded-lg border border-gray-200 bg-white p-3 text-[9px]">
      ...
      <div className="grid grid-cols-1 gap-2 2xl:grid-cols-2">
        ...
        <Panel title="Complexity & Automation Value">
          <div className="grid grid-cols-2 gap-3"><div><p className="text-[8px] font-semibold text-gray-500">Complexity</p><p className="mt-1 text-lg font-black text-gray-900">{classification.complexity_score ?? "—"}</p></div><div><p className="text-[8px] font-semibold text-gray-500">Automation value</p><p className="mt-1 text-lg font-black text-gray-900">{classification.automation_value_score ?? "—"}</p></div></div>
        </Panel>
      </div>
      ...
      {canApprove && <Panel title="Classification Decision" className="border-amber-200 bg-amber-50/40">
        {decisionKey && <div className="mb-2 rounded-md border border-gray-200 bg-white p-2"><label className="text-[9px] font-extrabold text-gray-700">Reason {CLASSIFICATION_DECISIONS.find((item) => item.key === decisionKey)?.reasonRequired ? "(required)" : "(optional)"}</label><textarea aria-label="Classification decision reason" value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1 h-16 w-full resize-none rounded border border-gray-200 p-2 text-[10px]" /><div className="mt-2 flex gap-2"><Button onClick={() => void submitDecision(decisionKey)} disabled={Boolean(busy) || (CLASSIFICATION_DECISIONS.find((item) => item.key === decisionKey)?.reasonRequired && !reason.trim())} className="h-8 flex-1 bg-[#B71920] text-[9px] font-bold text-white">Confirm</Button><Button variant="outline" onClick={() => { setDecisionKey(null); setReason(""); }} className="h-8 text-[9px] font-bold">Cancel</Button></div></div>}
...
function Timeline({ entries }: { entries: Array<{ id: string; at: string; actor: string; title: string; detail: string; comment?: string | null }> }) {
  return <div className="space-y-3">{entries.map((entry) => <div key={entry.id} className="relative border-l-2 border-app-brand-100 pl-4 before:absolute before:-left-[5px] before:top-1 before:h-2 before:w-2 before:rounded-full before:bg-app-brand-600"><p className="text-[8px] font-bold text-gray-500">{dateTime(entry.at)}</p><p className="mt-1 text-[9px] font-extrabold text-gray-800">{entry.title} · {entry.actor}</p><p className="mt-0.5 text-[9px] font-semibold text-gray-600">{entry.detail}</p>{entry.comment && <p className="mt-1 rounded bg-gray-50 p-1.5 text-[8px] font-semibold text-gray-500">{entry.comment}</p>}</div>)}</div>;
}

function LabelValue({ label, value }: { label: string; value: React.ReactNode }) {
  return <div className="min-w-0"><p className="text-[8px] font-semibold text-gray-400">{label}</p><p className="mt-1 break-words text-[9px] font-extrabold text-gray-800">{value}</p></div>;
}
