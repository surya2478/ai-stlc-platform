...
function RadialProgress({ rate, color }: { rate: number; color: string }) {
  ...
  return (
    <div className="relative flex items-center justify-center h-8 w-8 shrink-0 select-none">
      ...
      <span className="absolute text-[9px] font-bold text-gray-700">{rate}%</span>
    </div>
  );
}
...
function CommandCenterContent() {
  ...
  const pipelineSteps = useMemo(() => {
    ...
    return [
      { label: "Draft", count: draftCount, rate: draftPct, color: "#9CA3AF", icon: FileText, iconBg: "bg-gray-50 text-gray-500 border-gray-100" },
      ...
      { label: "Executed", count: executedCount, rate: executedPct, color: "#D52B31", icon: Play, iconBg: "bg-app-brand-75 text-app-brand-500 border-app-brand-100" },
  ...
  const priorityChartData = useMemo(() => {
    ...
    return [
      ...
      { name: "Info", value: info, color: "#D52B31", count: `${info} (${totalTCs > 0 ? ((info / totalTCs) * 100).toFixed(1) : 0}%)` },
    ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="rounded-xl bg-app-brand-75 border border-app-brand-100 p-2.5">
            <Layers className="h-6 w-6 text-[#B71920]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Test Cases Command Center</h1>
            <p className="text-xs text-gray-500 mt-1">AI-powered overview of test case lifecycle, approvals, automation readiness, and Jira traceability</p>
          </div>
        </div>
        
        <div className="flex items-center gap-2.5 shrink-0 self-end md:self-auto">
          <Button variant="outline" size="sm" onClick={loadData} className="h-8 w-8 p-0 border-gray-200 bg-white">
            <RefreshCw className={cn("h-3.5 w-3.5 text-gray-500", loading && "animate-spin")} />
          </Button>

          <div className="h-7 border-l border-gray-200 self-center hidden sm:block mx-1" />

          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider hidden sm:block">Last updated: {lastUpdatedDisplay}</span>
          
          <Button variant="default" size="sm" onClick={() => router.push(`/test-cases?project=${selectedProjectId}`)} className="h-8 text-xs bg-[#B71920] hover:bg-[#941216] text-white font-semibold">
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
            {stats.map((card) => {
              ...
              return (
                <Card key={card.title} className="border-gray-200 hover:-translate-y-0.5 transition-all bg-white shadow-sm">
                  <CardContent className="p-4 flex flex-col justify-between h-full space-y-3">
                    ...
                    <div className="flex items-baseline gap-1.5">
                      ...
                      {card.sublabel && (
                        <span className="text-[9px] font-bold text-gray-400">{card.sublabel}</span>
                      )}
                    </div>
                    <div className="text-[10px] text-gray-450 font-bold border-t border-gray-50 pt-2">
                      {card.footer}
                    </div>
                  </CardContent>
                </Card>
          ...
          <Card className="border-gray-200 bg-white shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b px-5 py-4 border-gray-100 bg-gray-50/50">
              <div>
                <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Test Case Lifecycle Overview</h3>
                <p className="text-[10px] text-gray-400 font-semibold mt-1">End-to-end test case workflow progress</p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => router.push(`/test-cases?project=${selectedProjectId}`)}
                className="h-8 text-xs border-gray-200 text-[#B71920] bg-white font-semibold"
              >
                View Test Cases
              </Button>
            </div>
                ...
                {pipelineSteps.map((step, idx) => {
                  ...
                  return (
                    <div key={step.label} className="flex items-center flex-1 gap-2 last:flex-none">
                      <div className="flex items-center gap-3 border rounded-xl p-3 bg-white border-gray-200/80 shadow-inner flex-1 max-w-[200px]">
                        <div className={cn("rounded-lg p-2 flex items-center justify-center shrink-0 border", step.iconBg)}>
                          <Icon className="h-4.5 w-4.5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider">{step.label}</p>
                          <p className="text-xs font-bold text-gray-800 mt-0.5">{step.count.toLocaleString()} / {totalTCs.toLocaleString()}</p>
                        </div>
                        <RadialProgress rate={step.rate} color={step.color} />
                      </div>
                      
                      {idx < pipelineSteps.length - 1 && (
                        <div className="flex items-center justify-center text-gray-300 font-bold shrink-0 px-1 select-none">
                          <ArrowRight className="h-4.5 w-4.5 text-gray-300" />
                        </div>
                      )}
                    </div>
                  );
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              <div className="flex items-center justify-between border-b px-5 py-4 border-gray-100">
                <div className="flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-app-brand-500" />
                  <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Test Case Trend</h3>
                </div>
                <select className="appearance-none bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-800 rounded-lg text-[10px] font-bold px-2.5 py-1 pr-6 focus:outline-none cursor-pointer"
                  style={{
              ...
              <div className="p-4 h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fontSize: 9, fill: "#9CA3AF", fontWeight: 700 }} stroke="#E8EAEE" />
                    <YAxis tick={{ fontSize: 9, fill: "#9CA3AF", fontWeight: 700 }} stroke="#E8EAEE" />
                    <ChartTooltip contentStyle={{ fontSize: "10px", fontWeight: "bold", borderRadius: "8px", border: "1px solid #E8EAEE" }} />
                    <ChartLegend wrapperStyle={{ fontSize: "10px", fontWeight: "bold", paddingTop: "10px" }} iconType="circle" />
                    <Line type="monotone" dataKey="Created" stroke="#D52B31" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} />
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              <div className="flex items-center justify-between border-b px-5 py-4 border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Priority Summary</h3>
                </div>
                <select className="appearance-none bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-800 rounded-lg text-[10px] font-bold px-2.5 py-1 pr-6 focus:outline-none cursor-pointer"
                  style={{
                    backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2364748b' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`,
              ...
              <div className="p-4 flex flex-row items-center justify-between gap-4 h-64">
                <div className="relative w-1/2 h-full flex items-center justify-center">
                  ...
                  <div className="absolute flex flex-col items-center justify-center select-none">
                    <span className="text-xl font-extrabold text-gray-800">{totalTCs.toLocaleString()}</span>
                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Total</span>
                  </div>
                </div>
                
                <div className="w-1/2 space-y-2 text-[10px] font-bold text-gray-600">
                  {priorityChartData.map((item) => (
                    <div key={item.name} className="flex items-center justify-between">
                      ...
                      <span className="text-gray-500 font-mono text-[9px]">{item.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              
              <div className="border-t border-gray-50 p-3 bg-gray-50/20 text-center">
                <button onClick={() => router.push(`/test-cases?project=${selectedProjectId}`)} className="text-[10px] font-bold text-[#B71920] hover:underline flex items-center justify-center gap-1 mx-auto">
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              <div className="flex items-center justify-between border-b px-5 py-4 border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Jira Sync Health</h3>
                </div>
                <select className="appearance-none bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-800 rounded-lg text-[10px] font-bold px-2.5 py-1 pr-6 focus:outline-none cursor-pointer"
                  style={{
                    backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2364748b' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`,
              ...
              <div className="p-5 space-y-5 flex-1 flex flex-col justify-center">
                <div className="flex items-start gap-3 border rounded-xl p-3 bg-emerald-50/50 border-emerald-100/50">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-gray-800">All systems are healthy</h4>
                    <p className="text-[10px] text-gray-400 font-semibold mt-0.5">Last synced 2 mins ago</p>
                  </div>
                </div>

                <div className="grid grid-cols-4 gap-2 text-center select-none font-bold">
                  <div className="bg-gray-50/50 border rounded-lg p-2.5">
                    <div className="text-sm font-extrabold text-gray-800">{jiraSyncedCount.toLocaleString()}</div>
                    <div className="text-[8px] text-gray-400 uppercase mt-0.5 tracking-wider">Synced</div>
                  </div>
                  <div className="bg-gray-50/50 border rounded-lg p-2.5">
                    <div className="text-sm font-extrabold text-gray-800">{jiraPendingCount.toLocaleString()}</div>
                    <div className="text-[8px] text-gray-400 uppercase mt-0.5 tracking-wider">Pending</div>
                  </div>
                  <div className="bg-gray-50/50 border rounded-lg p-2.5">
                    <div className="text-sm font-extrabold text-rose-600">{jiraFailedCount.toLocaleString()}</div>
                    <div className="text-[8px] text-gray-400 uppercase mt-0.5 tracking-wider">Failed</div>
                  </div>
                  <div className="bg-gray-50/50 border rounded-lg p-2.5">
                    <div className="text-sm font-extrabold text-amber-600">{jiraConflictCount.toLocaleString()}</div>
                    <div className="text-[8px] text-gray-400 uppercase mt-0.5 tracking-wider">Conflicts</div>
                  </div>
                </div>
              </div>

              <div className="border-t border-gray-50 p-3 bg-gray-50/20 text-center">
                <button onClick={() => router.push(`/settings?project=${selectedProjectId}`)} className="text-[10px] font-bold text-[#B71920] hover:underline flex items-center justify-center gap-1 mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
                <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">AI Agent Activity</h3>
              </div>
              <div className="divide-y divide-gray-100 flex-1">
                {agentActivity.map((item, index) => (
                  <div key={index} className="flex items-center gap-3 px-4 py-3 text-xs font-semibold text-gray-700">
                    <div className="rounded-lg bg-app-brand-75 border border-app-brand-100 p-1.5 text-app-brand-500 shrink-0">
                      <Cpu className="h-4 w-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-800 truncate">{item.name}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5 leading-normal">Scanned specifications and mapped actions</p>
                    </div>
                    <div className="flex flex-col items-end gap-1.5 shrink-0 select-none">
                      <Badge variant={getStatusVariant(item.status)} className="capitalize text-[9px] py-0 px-2">
                        {item.status}
                      </Badge>
                      <span className="text-[9px] text-gray-400 font-bold font-mono">{item.time}</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="border-t border-gray-100 p-3 bg-gray-50/30 text-center">
                <button onClick={() => router.push(`/agents?project=${selectedProjectId}`)} className="text-[10px] font-bold text-[#B71920] hover:underline flex items-center justify-center gap-1 mx-auto">
                  View All Agent Runs
                  <ChevronRight className="h-3 w-3" />
                </button>
              </div>
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
                <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Pending Approvals</h3>
              </div>
              <div className="divide-y divide-gray-100 flex-1">
                {pendingApprovals.map((item, index) => (
                  <div key={index} className="flex items-center justify-between gap-3 px-4 py-3 text-xs font-semibold text-gray-700">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="rounded-lg bg-gray-50 border border-gray-100 p-1.5 text-gray-400 shrink-0">
                        <FileText className="h-4 w-4" />
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-bold text-gray-800 truncate leading-snug">{item.title}</h4>
                        <span className="font-mono text-[9px] text-gray-400 font-bold block mt-0.5">{item.code}</span>
                      </div>
                    </div>
            ...
            <Card className="border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col justify-between">
              ...
              <div className="divide-y divide-gray-100 flex-1">
                {recentActivity.map((item, index) => (
                  <div key={index} className="flex items-center gap-3 px-4 py-3 text-xs font-semibold text-gray-700">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 text-gray-600 text-[10px] font-bold shrink-0">
                      {item.user.split(/\s+/).map(p => p[0]).join("").toUpperCase().substring(0, 2)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-gray-700 leading-normal">
                        <span className="font-bold text-gray-800">{item.user}</span> {item.action}{" "}
                        <span className="font-bold text-[#B71920] font-mono">{item.subject}</span>
                      </p>
                    </div>
              ...
              <div className="border-t border-gray-100 p-3 bg-gray-50/30 text-center">
                <button onClick={() => router.push(`/dashboard?project=${selectedProjectId}`)} className="text-[10px] font-bold text-[#B71920] hover:underline flex items-center justify-center gap-1 mx-auto">
                  View All Activity
                  <ChevronRight className="h-3 w-3" />
                </button>
              </div>
