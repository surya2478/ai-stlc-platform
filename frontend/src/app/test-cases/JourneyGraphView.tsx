...
const NODE_TONES: Record<NodeType, string> = {
  requirement: "border-gray-200 bg-white text-gray-800",
  journey: "border-app-brand-500 bg-app-brand-75 text-app-brand-900 shadow-sm shadow-app-brand-100",
  scenario: "border-emerald-200 bg-emerald-50/50 text-gray-800",
  test_case: "border-emerald-200 bg-white text-gray-800",
  application: "border-cyan-200 bg-cyan-50/40 text-gray-800",
  evidence: "border-violet-200 bg-violet-50/40 text-gray-800",
...
export function JourneyGraphView({ projectId }: Props) {
  ...
  if (!projectId) return <div className="flex h-64 items-center justify-center text-sm font-semibold text-gray-500">Select a project to load its journey graph.</div>;

  return (
    <div className="min-h-full">
      <section className="space-y-2 pb-4">
        <div className="flex items-center gap-2 text-xs font-semibold text-gray-500">
          <span>QAI Command Center</span><ChevronRight className="h-3 w-3 text-gray-300" /><span className="text-[#B71920]">Test Planning</span><ChevronRight className="h-3 w-3 text-gray-300" /><span className="text-gray-800">Journey Graph</span>
        </div>

        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2"><h1 className="text-2xl font-extrabold tracking-tight text-gray-950">Journey Graph</h1><span className="rounded-md border border-purple-100 bg-purple-50 px-2 py-0.5 text-[10px] font-bold text-purple-700">P1-S3 UI-012</span></div>
            <p className="mt-1 text-xs font-semibold text-gray-500">Visualize scenario coverage, journey paths, application touchpoints and approval readiness.</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden text-[10px] font-semibold text-gray-500 xl:inline">Last refreshed: {lastRefreshed ? displayDate(lastRefreshed.toISOString()) : "Loading"}</span>
            <Button aria-label="Refresh journey graph" variant="outline" size="sm" onClick={() => void loadData()} disabled={loading} className="h-9 gap-2 border-gray-200 text-xs font-bold"><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} />Refresh</Button>
            <Button aria-label="Rebuild journey graph" size="sm" onClick={() => void loadData()} disabled={loading} className="h-9 gap-2 bg-[#B71920] text-xs font-bold text-white hover:bg-[#941216]"><Network className="h-4 w-4" />Rebuild Graph</Button>
            <Button aria-label="Export journey graph" variant="outline" size="sm" onClick={exportGraph} disabled={!journeys.length} className="h-9 gap-2 border-gray-200 text-xs font-bold"><Download className="h-4 w-4" />Export Graph<ChevronDown className="h-3 w-3" /></Button>
          </div>
        </div>

        {(error || notice) && <div role="alert" className={cn("flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold", error ? "border-red-200 bg-red-50 text-red-700" : "border-app-brand-200 bg-app-brand-75 text-app-brand-700")}><AlertTriangle className="h-4 w-4 shrink-0" /><span className="flex-1">{error || notice}</span><button aria-label="Dismiss message" onClick={() => { setError(""); setNotice(""); }}><X className="h-4 w-4" /></button></div>}
        ...
        <div className="grid grid-cols-4 gap-x-4 gap-y-3 rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm xl:grid-cols-8">
        ...
        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center gap-1.5 overflow-hidden border-b border-gray-100 p-2">
            <label className="relative w-[250px] shrink-0"><Search className="absolute left-3 top-2 h-3.5 w-3.5 text-gray-400" /><input aria-label="Search journeys" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search requirement, journey, scenario, test case, PPM ID or app..." className="h-8 w-full rounded-md border border-gray-200 pl-9 pr-3 text-[10px] font-semibold outline-none focus:border-app-brand-300" /></label>
            ...
            <Button variant="outline" size="sm" onClick={() => setAdvancedOpen((value) => !value)} className="h-8 shrink-0 gap-1.5 border-gray-200 px-2 text-[9px] font-bold text-[#B71920]"><Filter className="h-3 w-3" />More Filters<ChevronDown className={cn("h-3 w-3 transition", advancedOpen && "rotate-180")} /></Button>
          </div>
          {advancedOpen && <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50/60 px-3 py-2 text-[11px] font-semibold text-gray-600"><span>Filters use live requirement, scenario, test-case and application fields.</span><button onClick={() => { setQuery(""); setDomainFilter("all"); setJourneyFilter("all"); setScenarioFilter("all"); setApplicationFilter("all"); setCoverageFilter("all"); setApprovalFilter("all"); }} className="font-bold text-[#B71920]">Clear all filters</button></div>}

          <div className="flex items-center justify-between border-b border-gray-100 px-3 py-2">
            <div className="flex items-center gap-1.5"><span className="text-xs font-extrabold text-gray-900">Journey Graph Canvas</span><CircleDot className="h-3.5 w-3.5 text-gray-400" /></div>
            <div className="flex items-center gap-2">
              <label className="relative"><Search className="absolute left-2.5 top-2 h-3.5 w-3.5 text-gray-400" /><input aria-label="Search graph" value={graphQuery} onChange={(event) => setGraphQuery(event.target.value)} placeholder="Search graph..." className="h-8 w-28 rounded-md border border-gray-200 pl-8 pr-2 text-[10px] outline-none focus:border-app-brand-300" /></label>
              ...
              <div className="relative"><button aria-label="Graph options" onClick={() => setGraphOptionsOpen((value) => !value)} className="flex h-8 w-8 items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:bg-gray-50"><MoreVertical className="h-4 w-4" /></button>{graphOptionsOpen && <div className="absolute right-0 top-9 z-20 w-40 rounded-md border border-gray-200 bg-white p-1 shadow-lg"><button onClick={() => { setZoom(1); setGraphQuery(""); setShowGapsOnly(false); setShowEvidenceLinks(true); setShowApplicationLinks(true); setGraphOptionsOpen(false); }} className="w-full rounded px-2 py-2 text-left text-[9px] font-bold text-gray-700 hover:bg-gray-50">Reset graph controls</button><button onClick={() => router.push(`/test-cases?project=${projectId}&view=editor`)} className="w-full rounded px-2 py-2 text-left text-[9px] font-bold text-[#B71920] hover:bg-app-brand-75">Open Test Case Editor</button></div>}</div>
            </div>
          </div>

          <div className="h-[248px] overflow-hidden bg-gray-50/20 p-3">
            {loading ? <div className="flex h-full items-center justify-center gap-2 text-xs font-semibold text-gray-500"><Loader2 className="h-4 w-4 animate-spin text-[#B71920]" />Loading project graph...</div> : !selectedJourney ? <div className="flex h-full items-center justify-center text-xs font-semibold text-gray-500">No approved requirements are available to build a journey graph.</div> : (
          ...
          <div className="flex flex-wrap items-center gap-4 border-t border-gray-100 px-3 py-2 text-[9px] font-semibold text-gray-600"><span className="font-extrabold text-gray-800">Legend:</span><Legend color="bg-emerald-500" label="Positive" /><Legend color="bg-red-500" label="Negative" /><Legend color="bg-amber-500" label="Boundary" /><Legend color="bg-purple-500" label="Recovery" /><Legend color="bg-app-brand-500" label="Regression" /><Legend color="bg-yellow-400" label="Automation Candidate" /><Legend line="border-gray-400" label="maps to" /><Legend line="border-emerald-400" label="validates" /><Legend line="border-app-brand-500" label="covers" /><Legend line="border-purple-400" label="requires evidence" /><Legend line="border-red-400" label="gap / blocker" /></div>
        </div>

        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b border-gray-100 px-3 py-2"><span className="text-xs font-extrabold text-gray-900">Journey Coverage List</span><CircleDot className="h-3.5 w-3.5 text-gray-400" /><span className="rounded-full bg-gray-100 px-2 py-0.5 text-[9px] font-bold text-gray-600">{filteredJourneys.length} of {journeys.length}</span></div>
          <div className="grid grid-cols-[76px_minmax(150px,1.2fr)_82px_82px_82px_110px_112px_52px_120px_98px_130px] border-b border-gray-100 bg-gray-50/70 px-3 py-1.5 text-[8px] font-extrabold uppercase text-gray-500"><span>Journey ID</span><span>Journey Name</span><span>Requirement Count</span><span>Scenario Count</span><span>Test Case Count</span><span>Application Mappings</span><span>Evidence Coverage</span><span>Gaps</span><span>Approval Readiness</span><span>Owner</span><span>Updated At</span></div>
          {visibleJourneys.map((journey) => <button key={journey.id} onClick={() => selectJourney(journey)} className={cn("grid w-full grid-cols-[76px_minmax(150px,1.2fr)_82px_82px_82px_110px_112px_52px_120px_98px_130px] items-center border-b border-gray-100 px-3 py-1.5 text-left text-[9px] font-semibold text-gray-700 transition hover:bg-app-brand-75/50", selectedJourney?.id === journey.id && "border-l-4 border-l-[#B71920] bg-app-brand-75/70 pl-2")}><span className="font-bold text-[#B71920]">{journey.id}</span><span className="truncate font-bold text-gray-900">{journey.name}</span><span>{journey.requirements.length}</span><span>{journey.scenarios.length}</span><span>{journey.testCases.length}</span><span className="font-bold text-[#B71920]">{journey.applications.length}/{Math.max(journey.applications.length, new Set(journey.testCases.map((item) => item.application_id).filter(Boolean)).size)}</span><ProgressCell value={journey.evidenceCoverage} /><span><span className={cn("rounded-full px-2 py-0.5 font-bold", journey.gaps.length ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600")}>{journey.gaps.length}</span></span><ProgressCell value={journey.approvalReadiness} /><span>{journey.ownerId ? `User #${journey.ownerId}` : "Unassigned"}</span><span className="text-gray-500">{displayDate(journey.updatedAt)}</span></button>)}
          {!visibleJourneys.length && <div className="p-8 text-center text-xs font-semibold text-gray-500">No journeys match the selected filters.</div>}
          <div className="flex items-center justify-between px-3 py-2 text-[10px] font-semibold text-gray-500"><span>Showing {visibleJourneys.length ? (page - 1) * pageSize + 1 : 0} to {Math.min(page * pageSize, filteredJourneys.length)} of {filteredJourneys.length} journeys</span><div className="flex items-center gap-1"><button aria-label="Previous journey page" disabled={page === 1} onClick={() => setPage((value) => Math.max(1, value - 1))} className="flex h-7 w-7 items-center justify-center rounded border border-gray-200 disabled:opacity-40"><ChevronLeft className="h-3.5 w-3.5" /></button><span className="flex h-7 min-w-7 items-center justify-center rounded border border-app-brand-500 bg-app-brand-75 px-2 font-bold text-[#B71920]">{page}</span><button aria-label="Next journey page" disabled={page === pages} onClick={() => setPage((value) => Math.min(pages, value + 1))} className="flex h-7 w-7 items-center justify-center rounded border border-gray-200 disabled:opacity-40"><ChevronRight className="h-3.5 w-3.5" /></button><span className="ml-2 rounded border border-gray-200 px-3 py-1.5">{pageSize} / page</span></div></div>
        </div>
      </section>
      ...
      <DrawerContent size="lg">
        <DrawerHeader>
          <div>
            ...
            <p className="mt-2 text-xs font-bold text-gray-800">{selectedNode?.label || selectedJourney?.name || "No journey selected"}</p>
          </div>
          <button aria-label="Close inspector" onClick={() => setInspectorOpen(false)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-50"><X className="h-4 w-4" /></button>
        </DrawerHeader>
        <div className="grid grid-cols-6 border-b border-gray-100 px-2 shrink-0">{(["overview", "coverage", "applications", "evidence", "automation", "activity"] as InspectorTab[]).map((tab) => <button key={tab} onClick={() => setInspectorTab(tab)} className={cn("border-b-2 px-1 py-3 text-[9px] font-bold capitalize", inspectorTab === tab ? "border-[#B71920] text-[#B71920]" : "border-transparent text-gray-500")}>{tab}</button>)}</div>
        <DrawerBody className="space-y-3">
          ...
          {selectedJourney && <>
            ...
            {selectedNode?.type === "gap" && <Button variant="outline" onClick={() => void markGapReviewed(selectedNode.raw as GraphGap)} disabled={busy === "gap"} className="h-9 w-full border-gray-300 text-[10px] font-bold">Mark Gap Reviewed</Button>}
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" onClick={() => void addMissingScenarios()} disabled={busy === "scenario" || !selectedJourney.requirements.length} className="h-9 gap-1 border-app-brand-300 text-[10px] font-bold text-[#B71920]"><Plus className="h-3.5 w-3.5" />Add Missing Scenario</Button>
            ...
            <Button onClick={() => void sendToApproval()} disabled={selectedJourney.gaps.length > 0 || busy === "approval" || !selectedJourney.testCases.length} className="h-10 w-full bg-[#B71920] text-[11px] font-bold text-white hover:bg-[#941216]"><ShieldCheck className="mr-2 h-4 w-4" />Send to Test Case Approval</Button>
...
function Kpi({ title, value, subtitle, icon: Icon, tone, progress }: { title: string; value: string | number; subtitle: string; icon: typeof FileText; tone: "emerald" | "blue" | "purple" | "cyan" | "amber" | "green"; progress?: number }) {
  const tones = { emerald: "border-emerald-200 bg-emerald-50 text-emerald-600", blue: "border-app-brand-200 bg-app-brand-75 text-app-brand-600", purple: "border-purple-200 bg-purple-50 text-purple-600", cyan: "border-cyan-200 bg-cyan-50 text-cyan-600", amber: "border-amber-200 bg-amber-50 text-amber-600", green: "border-green-200 bg-green-50 text-green-600" };
  return <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm"><div className="flex items-center gap-2"><span className={cn("flex h-7 w-7 items-center justify-center rounded-lg border", tones[tone])}><Icon className="h-3.5 w-3.5" /></span><p className="truncate text-[9px] font-extrabold text-gray-800">{title}</p></div><div className="mt-2 flex items-end gap-2"><div><p className="text-lg font-extrabold text-gray-950">{value}</p><p className="mt-0.5 text-[8px] font-semibold text-gray-500">{subtitle}</p></div>{progress != null && <div className="ml-auto h-8 w-8 rounded-full p-1" style={{ background: `conic-gradient(#55a630 ${progress * 3.6}deg, #E8EAEE 0deg)` }}><div className="h-full w-full rounded-full bg-white" /></div>}</div></div>;
}

function Readiness({ label, value, good, blocker }: { label: string; value: string; good: boolean; blocker?: boolean }) {
  return <div className="flex min-w-0 items-center gap-2"><span className={cn("flex h-5 w-5 shrink-0 items-center justify-center rounded-full border", good ? "border-emerald-200 bg-emerald-50 text-emerald-600" : blocker ? "border-red-200 bg-red-50 text-red-600" : "border-amber-200 bg-amber-50 text-amber-600")}>{good ? <CheckCircle2 className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}</span><div className="min-w-0"><p className="truncate text-[8px] font-bold text-gray-600">{label}</p><p className="text-[9px] font-extrabold text-gray-900">{value}</p></div></div>;
}

function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string }> }) {
  return <select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="h-8 min-w-0 flex-1 rounded-md border border-gray-200 bg-white px-1.5 text-[9px] font-semibold text-gray-700 outline-none focus:border-app-brand-300"><option value="all">{label}: All</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>;
}

function GraphButton({ label, icon: Icon, onClick }: { label: string; icon: typeof Search; onClick: () => void }) { return <button onClick={onClick} className="flex h-8 items-center gap-1 rounded-md border border-gray-200 px-2 text-[9px] font-bold text-gray-600 hover:bg-gray-50"><Icon className="h-3.5 w-3.5" />{label}</button>; }

function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) { return <label className="flex h-8 cursor-pointer items-center gap-1.5 text-[9px] font-bold text-gray-600"><button type="button" role="switch" aria-label={label} aria-checked={value} onClick={() => onChange(!value)} className={cn("relative h-4 w-7 rounded-full transition", value ? "bg-[#B71920]" : "bg-gray-200")}><span className={cn("absolute top-0.5 h-3 w-3 rounded-full bg-white transition", value ? "left-3.5" : "left-0.5")} /></button>{label}</label>; }

function GraphColumn({ title, children, hidden }: { title: string; children: React.ReactNode; hidden?: boolean }) { return <div className={cn("min-w-0", hidden && "opacity-15")}><p className="mb-1.5 text-center text-[8px] font-extrabold text-gray-900">{title}</p><div className="space-y-1.5">{children}</div></div>; }

function Connector({ label, hidden }: { label: string; hidden?: boolean }) { return <div className={cn("pt-12 text-center", hidden && "opacity-15")}><p className="-ml-5 mb-1 w-16 text-[7px] font-semibold text-gray-500">{label}</p><div className="border-t border-dashed border-gray-400" /></div>; }
...
function GraphNode({ type, title, detail, selected, scenario, classificationStatus, onClick }: { type: NodeType; title: string; detail: string; selected?: boolean; scenario?: ScenarioKind; classificationStatus?: string; onClick: () => void }) {
  const scenarioTone = scenario === "negative" ? "border-red-300 bg-red-50" : scenario === "boundary" ? "border-amber-300 bg-amber-50" : scenario === "recovery" ? "border-purple-300 bg-purple-50" : scenario === "regression" ? "border-app-brand-300 bg-app-brand-75" : "";
  return <button onClick={onClick} className={cn("relative w-full rounded-md border p-1.5 text-left transition hover:ring-2 hover:ring-app-brand-100", NODE_TONES[type], scenarioTone, selected && "ring-2 ring-app-brand-200")}>
    {classificationStatus && <span title={`Automation classification: ${classificationStatus.replace(/_/g, " ")}`} className={cn("absolute right-1 top-1 h-1.5 w-1.5 rounded-full", CLASSIFICATION_DOT_TONE[classificationStatus] || "bg-gray-300")} />}
...
function EmptyNode({ label, tone = "slate" }: { label: string; tone?: "slate" | "red" }) { return <div className={cn("rounded-md border border-dashed p-2 text-center text-[8px] font-bold", tone === "red" ? "border-red-300 bg-red-50 text-red-600" : "border-gray-300 text-gray-400")}>{label}</div>; }
function MoreCount({ count }: { count: number }) { return count > 0 ? <div className="mx-auto w-fit rounded-full bg-gray-100 px-3 py-1 text-[8px] font-bold text-gray-600">+{count} more</div> : null; }
...
function ProgressCell({ value }: { value: number }) { const tone = value >= 80 ? "bg-emerald-500" : value >= 50 ? "bg-amber-500" : "bg-red-500"; return <span className="flex items-center gap-2"><span className="h-1.5 w-14 rounded-full bg-gray-100"><span className={cn("block h-full rounded-full", tone)} style={{ width: `${value}%` }} /></span><span>{value}%</span></span>; }

function Panel({ title, action, onAction, tone, children }: { title: string; action?: string; onAction?: () => void; tone?: "red"; children: React.ReactNode }) { return <section className={cn("rounded-lg border p-3", tone === "red" ? "border-red-200 bg-red-50/50" : "border-gray-200 bg-white")}><div className="mb-3 flex items-center justify-between"><h3 className="text-[10px] font-extrabold text-gray-800">{title}</h3>{action && <button onClick={onAction} className="text-[9px] font-bold text-[#B71920]">{action}</button>}</div>{children}</section>; }

function InfoPair({ label, value, link }: { label: string; value: string; link?: boolean }) { return <div><p className="text-[8px] font-bold text-gray-400">{label}</p><p className={cn("mt-1 break-words text-[10px] font-bold", link ? "text-[#B71920]" : "text-gray-700")}>{value}</p></div>; }
...
function Ring({ label, value, locked }: { label: string; value: number; locked?: boolean }) { return <div className="text-center"><div className="mx-auto h-8 w-8 rounded-full p-1" style={{ background: locked ? "#fee2e2" : `conic-gradient(${value >= 75 ? "#10b981" : "#f59e0b"} ${value * 3.6}deg, #E8EAEE 0deg)` }}><div className="flex h-full w-full items-center justify-center rounded-full bg-white text-[7px] font-extrabold text-gray-700">{locked ? "Gated" : `${value}%`}</div></div><p className="mt-1 text-[8px] font-bold text-gray-500">{label}</p></div>; }
...
function ApplicationsInspector({ projectId, journey, applications, availableEnvironments, environment, setEnvironment, mappingFormOpen, setMappingFormOpen, applicationId, setApplicationId, onResolve, busy }: { projectId: number; journey: JourneyRecord; applications: ProjectApplication[]; availableEnvironments: string[]; environment: string; setEnvironment: (value: string) => void; mappingFormOpen: boolean; setMappingFormOpen: (value: boolean) => void; applicationId: string; setApplicationId: (value: string) => void; onResolve: () => void; busy: string }) {
  return <>
    <Panel title="Mapped Applications"><div className="space-y-2">{journey.applications.map((item) => <div key={item.key} className="rounded-md border border-cyan-100 bg-cyan-50/50 p-2"><p className="text-[10px] font-extrabold text-gray-800">{item.name}</p><p className="mt-1 text-[9px] font-semibold text-gray-500">Registry key: {item.key}</p><p className="mt-1 text-[9px] font-semibold text-gray-500">{Object.keys(item.environment_urls).length} environment URL(s)</p></div>)}{!journey.applications.length && <p className="text-[10px] font-semibold text-red-600">No active application is mapped.</p>}</div></Panel>
    <Panel title="Discovery Environment"><select aria-label="Discovery environment" value={environment} onChange={(event) => setEnvironment(event.target.value)} className="h-9 w-full rounded-md border border-gray-200 px-2 text-[10px]"><option value="QA">QA</option>{availableEnvironments.filter((item) => item !== "QA").map((item) => <option key={item} value={item}>{item}</option>)}</select></Panel>
    <button aria-label="Open application mapping form" onClick={() => setMappingFormOpen(!mappingFormOpen)} className="flex items-center gap-1 text-[10px] font-bold text-[#B71920]"><Link2 className="h-3.5 w-3.5" />Resolve Application Mapping</button>
    {mappingFormOpen && <Panel title="Resolve Mapping"><select aria-label="Application mapping" value={applicationId} onChange={(event) => setApplicationId(event.target.value)} className="h-9 w-full rounded-md border border-gray-200 px-2 text-[10px]"><option value="">Select application</option>{applications.filter((item) => item.id != null).map((item) => <option key={item.key} value={String(item.id)}>{item.name}</option>)}</select><Button onClick={onResolve} disabled={!applicationId || busy === "mapping"} className="mt-2 h-9 w-full bg-[#B71920] text-[10px] font-bold text-white">Apply Mapping</Button></Panel>}
...
function EvidenceInspector({ journey, evidenceFormOpen, setEvidenceFormOpen, evidenceText, setEvidenceText, onSave, busy }: { journey: JourneyRecord; evidenceFormOpen: boolean; setEvidenceFormOpen: (value: boolean) => void; evidenceText: string; setEvidenceText: (value: string) => void; onSave: () => void; busy: string }) {
  ...
  return <><Panel title="Evidence Requirements"><div className="space-y-2">{evidence.map((item) => <div key={item} className="flex items-start gap-2 rounded-md border border-purple-100 bg-purple-50/50 p-2 text-[10px] font-semibold text-gray-700"><FileCheck2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-purple-600" />{item}</div>)}{!evidence.length && <p className="text-[10px] font-semibold text-red-600">No evidence requirements are attached.</p>}</div></Panel><button aria-label="Open evidence requirement form" onClick={() => setEvidenceFormOpen(!evidenceFormOpen)} className="flex items-center gap-1 text-[10px] font-bold text-[#B71920]"><Plus className="h-3.5 w-3.5" />Add Evidence Requirement</button>{evidenceFormOpen && <Panel title="New Evidence Requirement"><textarea aria-label="Evidence requirement" value={evidenceText} onChange={(event) => setEvidenceText(event.target.value)} placeholder="Describe the required screenshot, log, trace or audit evidence." className="min-h-20 w-full rounded-md border border-gray-200 p-2 text-[10px] outline-none focus:border-app-brand-300" /><Button onClick={onSave} disabled={!evidenceText.trim() || busy === "evidence"} className="mt-2 h-9 w-full bg-[#B71920] text-[10px] font-bold text-white">Save Evidence Requirement</Button></Panel>}</>;
}
...
function AutomationInspector({ journey, selectedNode, classifications, enabled }: { journey: JourneyRecord; selectedNode: SelectedNode | null; classifications: Map<number, TestCaseAutomationClassification>; enabled: boolean }) {
  if (!enabled) return <Panel title="Automation Classification"><p className="text-[10px] font-semibold text-gray-400">Automation classification is not enabled for this project.</p></Panel>;
  ...
  if (focusedCase) {
    if (!focusedClassification) {
      return <Panel title={`Automation Classification — ${focusedCase.test_case_id}`}><p className="text-[10px] font-semibold text-gray-400">Not yet classified. Classify from Generated Test Cases.</p></Panel>;
    }
    return <>
      ...
      <Panel title="Mandatory / Optional Validators"><div className="flex flex-wrap gap-1.5">{focusedClassification.mandatory_validators.map((item) => <span key={`m-${item}`} className="rounded-md border border-red-200 bg-red-50 px-2 py-0.5 text-[9px] font-bold text-red-700">{item}</span>)}{focusedClassification.optional_validators.map((item) => <span key={`o-${item}`} className="rounded-md border border-app-brand-200 bg-app-brand-75 px-2 py-0.5 text-[9px] font-bold text-app-brand-700">{item}</span>)}{!focusedClassification.mandatory_validators.length && !focusedClassification.optional_validators.length && <span className="text-[9px] font-semibold text-gray-400">None declared</span>}</div></Panel>
    ...
    <div className="space-y-2">
      {journey.testCases.filter((item) => item.automation_candidate).map((item) => {
        ...
        return <div key={item.id} className="flex items-center justify-between text-[10px] font-semibold">
          <span className="text-gray-700">{item.test_case_id}</span>
      ...
      {!journey.testCases.some((item) => item.automation_candidate) && <p className="text-[10px] font-semibold text-gray-400">No automation candidates in this journey.</p>}
    </div>
  </Panel>;
}
...
function ActivityInspector({ journey, reviews, approvals }: { journey: JourneyRecord; reviews: ArtifactReview[]; approvals: ApprovalAction[] }) {
  ...
  return <Panel title="Audit Activity"><div className="space-y-3">{entries.slice(0, 12).map((entry, index) => <div key={`${entry.time}-${index}`} className="relative pl-4 text-[10px]"><span className="absolute left-0 top-1 h-2 w-2 rounded-full bg-[#B71920]" /><p className="font-bold text-gray-500">{displayDate(entry.time)}</p><p className="mt-0.5 font-extrabold text-gray-800">{entry.actor}</p><p className="font-semibold text-gray-600">{entry.text}</p></div>)}{!entries.length && <p className="text-[10px] font-semibold text-gray-500">No persisted review or approval activity is available for these test cases.</p>}</div></Panel>;
}

function GapCount({ label, value, tone }: { label: string; value: number; tone: "red" | "amber" | "yellow" | "blue" }) { const tones = { red: "border-red-200 bg-red-50 text-red-700", amber: "border-orange-200 bg-orange-50 text-orange-700", yellow: "border-amber-200 bg-amber-50 text-amber-700", blue: "border-app-brand-200 bg-app-brand-75 text-app-brand-700" }; return <div className={cn("rounded-md border p-2 text-center", tones[tone])}><p className="text-[8px] font-bold">{label}</p><p className="mt-1 text-base font-extrabold">{value}</p></div>; }
