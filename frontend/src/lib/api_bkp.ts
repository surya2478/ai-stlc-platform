...
const DEV_AUTH_EMAIL = process.env.NEXT_PUBLIC_DEV_AUTH_EMAIL ?? "";
const DEV_AUTH_PASSWORD = process.env.NEXT_PUBLIC_DEV_AUTH_PASSWORD ?? "";
...
function isDevAuthEnabled() {
  return (
    ...
    isLocalBrowser() &&
    Boolean(DEV_AUTH_EMAIL) &&
    Boolean(DEV_AUTH_PASSWORD)
  );
}
...
async function requestDevToken() {
  ...
  if (!devAuthPromise) {
    devAuthPromise = (async () => {
      ...
      form.set("username", DEV_AUTH_EMAIL);
      form.set("password", DEV_AUTH_PASSWORD);
...
api.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
  if (config.url?.includes("/users/token")) return config;

  const token =
    typeof window !== "undefined"
      ? window.localStorage.getItem(TOKEN_STORAGE_KEY) ?? (await requestDevToken())
      : null;
...
api.interceptors.response.use(
  ...
  async (error) => {
    if (error?.response?.status === 401) {
      ...
      if (
        originalConfig &&
        !originalConfig._retryDevAuth &&
        !originalConfig.url?.includes("/users/token") &&
        isDevAuthEnabled()
      ) {
        originalConfig._retryDevAuth = true;
      ...
      console.warn(
        ...
        "Sign in at /login, or enable local dev auth with explicit credentials."
      );
